...
| 画面 | `/races?date=YYYY-MM-DD`（ライブモード＝当日 snapshot あり） |
...
### TC-02: 縮退モード（snapshot 無し日）の発走列表示（#391）
| 項目 | 内容 |
|------|------|
| 前提 | `/api/live/{date}` が 0 件（predict-watch 未稼働日＝前日データ確認時など）で liveMode=false の静的一覧に縮退する |
| 画面 | `/races?date=YYYY-MM-DD`（縮退モード） |
| 操作 | 画面を開く |
| 期待結果 | 静的一覧のヘッダが「R / 開催 / 発走 / 距離 / 馬場 / 状態」で、発走列に race_cards 由来の HH:MM が表示される（post_time 未取得は「—」） |
| 確認ポイント | 縮退テーブルも `/api/races` の post_time を表示に使う。発走列の欠落がないこと |
