...
| 項目 | 内容 |
|------|------|
| 前提 | DB に race_card / スタッツ / オッズが存在し、EV > 1.0 の買い目が少なくとも 1 つある |
| コマンド | `paddock-analyze predict <有効な race_id>` |
| 期待結果 | 確率表に加えて「買い目推奨」セクションが表示される |
| 確認ポイント | 表示される全買い目の `ev = probability × odds` が `ev_threshold (1.0)` を超えていること / 馬連が先頭付近に表示される（優先度順） / kelly_fraction が 0.0〜0.25 の範囲内 |
...
| 項目 | 内容 |
|------|------|
| 前提 | 特定の三連単組み合わせのオッズが非常に高い（EV > 2.0） |
| コマンド | `paddock-analyze predict <対象 race_id>` |
| 期待結果 | 三連単が買い目推奨に含まれる |
| 確認ポイント | 三連単の EV が 2.0 を超えていること / 馬連・馬単・三連複の後に表示されること（優先度順） |
