...
| 確認ポイント | 推奨額の合計が budget を超えないこと / 残高が「初期予算 − 賭け金合計 + 払戻合計」と一致すること / 全レース終了後に一日集計が表示されること |
...
| 期待結果 | 「この日の開催はありません」等のメッセージが表示されて正常終了 |
| 確認ポイント | exit code 0 で終了すること / エラー出力なし |
...
| コマンド | `paddock-predict --date <開催日> --budget 10000` |
| 操作 | オッズ未取得レースで `s` を選択 |
| 期待結果 | 「オッズ未取得 — スキップ推奨」が表示され、他のレースは正常に処理される |
| 確認ポイント | 未取得レースでパニックしないこと / セッション全体が継続すること |
