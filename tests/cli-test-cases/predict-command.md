...
| 確認ポイント | 表示される全買い目の `ev = probability × odds` が `ev_threshold (1.0)` を超えていること / 馬連が先頭付近に表示される（優先度順） / kelly_fraction が `0.0 ≤ kelly_fraction ≤ kelly_cap (0.25)` の範囲内（境界値 0.0 および 0.25 は正常） |
