...
| 項目 | 内容 |
|------|------|
| 前提 | 障害レースの netkeiba race_id（例 `202607020609` = 2026-08-09 中京9R）。ADR 0075 |
| コマンド | `paddock-fetch-card 202607020609; echo $?` |
| 期待結果 | stdout に「スキップ: 障害レースは取り込み対象外です（取り込み失敗ではありません。…）」が出て **exit code 0**。DB は無変更 |
| 確認ポイント | `echo $?` が **0**（従来は 1）/ `Error:` 行が出ない / `race_cards`・`horse_entries`・`race_odds`・`fetch_history` に行が増えない / 近走（`horses`・`horse_past_runs`）も増えない＝後続処理が走っていない / 再実行しても同じ出力（冪等） |

> 回帰の相方: TC-06（不正 race_id）と TC-07（取得失敗）が **exit 1 のまま**であること、平地レース（TC-01）が
> 従来どおり取り込めることを同時に確認する。exit 0 の適用範囲が広がっていないことが本ケースの主眼。

**実施記録（2026-08-11・#586 の PR で実行）**: `202607020609`（2026-08-09 中京9R・障害）に対し exit 0、
stdout にスキップ行、`Error:` 行なし。`race_cards` / `horse_entries` / `race_odds` / `fetch_history` /
`horses` / `horse_past_runs` の 6 テーブルは実行前後で行数完全一致（`587|7834|1610994|995|4888|51180`）。
再実行しても同じ出力・exit 0（冪等）。回帰として `9999`（不正 race_id）と NAR の 12 桁 id はいずれも
exit 1 のままであることを確認済み。
