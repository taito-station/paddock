...
`pdfs/inbox/` 配下のファイルを引数にした場合、取り込みが成功した PDF は `pdfs/done/` へ自動的に移動される（未取り込みファイルが一目で分かるようにするため）。判定は src の親ディレクトリ構造 (`<root>/pdfs/inbox/<file>`) で行うので CWD に依存しない。`samples/` などインボックス外のパスは移動されない。

複数ファイル指定時、1 件でも ingest 失敗があれば終了コードが非 0 になる（成功分の取り込みと移動は維持される）。move 自体に失敗した場合は warn のみで ingest は成功扱い（ingest は `race_id` 単位で UPSERT するため、再実行しても重複は生じない）。
