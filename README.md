...
**保存・閲覧（ingest-predictions / api-server + web）**

- 予想（印・短評・買い目・結果）の DB 保存 — `ingest-predictions`
- REST API（actix-web）+ React SPA によるライブ盤面・予想閲覧 — `api-server` / `web/`
...
- **docker ランタイム（colima 推奨）**: compose の `postgres` / `api` / `web` / `importer` サービスと
  ...
  brew services start colima        # ログイン時に自動起動（推奨）
  docker compose version            # v2 サブコマンドが通ることを確認
  ```
  Docker Desktop でも代替可能。**Mac 再起動後は colima が立ち上がるまで Postgres・API・backup が
  全停止する**。`brew services start colima` を登録済みであれば再起動後に自動復旧する。手動で
  起こす場合は `colima start` を先に実行し、その後「[DB（Postgres）](#dbpostgres)」の起動コマンド
  （`docker compose -f deployments/compose.yaml up -d postgres`）を実行する。compose の各サービスは
  `restart: unless-stopped` を持つため、colima が起動すれば docker が既存コンテナを自動再起動する
  （compose の再実行は不要なことが多い。起動順序は healthcheck + `depends_on` で担保される）。
...
起動時に通知設定が 1 行で宣言される。一次情報はログの `🔔` 行（配送できなかったものは `🔔(未配送)`。配送できた件数は `grep -c '^  🔔 '` で数える——注記や警告の文中にも 🔔 が出るので行頭で絞る）。

- `--window <分>`: 発走まで残りこの時間以内のレースだけオッズを再取得する先読み窓（既定 40）。
- `--interval <分>`: スイープ間隔（既定 5）。
- `--roi-gate <倍率>`: 🔶 買い妙味として目立たせる ROI 閾値（既定 1.0 = 100%）。
- `--notify-gate <倍率>`: 🔍 検証候補として**ログ表示に**残す閾値。未指定なら `min(roi_gate, 0.7)`。明示指定は roi_gate 以下であること。**macOS 通知は鳴らさない**（鳴らすのは下の `--notify-roi`）。
- `--notify-roi <倍率>`: **macOS 通知の発火閾値**。未指定は `--roi-gate` と同値。同一レースは前回通知時から +10pt 上振れするまで再通知しない。既定値は ADR 0076 の実測どおり通過 0 件なので、#571 のゲート較正までは手で下げて実地検証する。
- `--no-notify`: macOS 通知を無効にする（既定は有効）。判定行の表示は変わらない。
...
通知の抑制状態はプロセス内メモリのみなので、`--once` を cron で繰り返す運用では毎回すべてのレースが初回扱いになる（常駐で回すのが前提）。
...
- API は `/api/*`（races / board / predictions / analyze / live / sessions 等）。`/docs/` に Swagger UI、
  `/api-docs/openapi.json` に OpenAPI ドキュメントを配信する（`/docs` は `/docs/` へリダイレクト）。
...
predict            … 1 日分を対話的に予想・購入記録（--budget で開始）
  ├─ ingest-predictions … 予想（印・買い目）を DB に保存 → api-server + web で閲覧
