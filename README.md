...
- 1 レースの win/place/show 確率推定（馬・騎手・調教師・コース・馬場・前走フォーム・斤量などの
  シグナルを重み付き平均し、ベイズ縮約・市場オッズブレンドで補正）
- 期待値・Kelly 配分にもとづく買い目推奨（単複・馬連・馬単・三連複・三連単）
- 1 開催日を対話的に予想して賭け金・払い戻し・収支を記録するセッション
- 日付範囲での backtest 検証（的中率・回収率・Brier・LogLoss）
...
- **tesseract** + **tesseract-lang**（jpn パック）: 成績 PDF の **着順の検証 override** と、mutool が
  ...
  決定的に取得する（#124 / ADR 0018）。OCR 経路はこれらに対しては冗長だが、着順は依然 OCR 由来の値で
  上書きする余地があるため tesseract は必須のまま（撤去は別 PR で検討）。
...
2. `mutool draw -F text` で PDF テキストを抽出し、土台となる Race / 結果テーブルを構築（着順は完走の行順から確定）
...
```bash
cargo run -p analyze -- predict 2026060412R02
cargo run -p analyze -- predict 2026060412R02 --blend-alpha 0.7        # 市場オッズ(単勝)をブレンド（#72）
cargo run -p analyze -- predict 2026060412R02 --track-condition 稍重    # 当日の馬場を factor に加える（#73）
```

- `--blend-alpha <α>`: モデル確率 α と市場 implied 確率 (1-α) の線形ブレンド。未指定はモデルのみ。
...
`predict` / `analyze predict` で買い目推奨まで出せる。

```bash
# netkeiba の race_id（12 桁）を直接指定
cargo run -p fetch-card -- 202606040812

# 構成要素から組み立て（全指定が必要）
cargo run -p fetch-card -- --year 2026 --venue tokyo --round 3 --day 6 --race 11

cargo run -p fetch-card -- 202606040812 --force          # 取得済みカードを上書き
cargo run -p fetch-card -- 202606040812 --skip-history   # 過去走の取り込みを省略
cargo run -p fetch-card -- 202606040812 --interval 1500  # リクエスト間隔(ms)
```
...
```bash
cargo run -p fetch-history -- 202606040811 202606040812   # race_id（複数可）
cargo run -p fetch-history -- --horse-id 2021104500       # 出馬表をバイパスして horse_id 直指定
cargo run -p fetch-history -- 202606040811 --no-backfill  # pdf 成績行への horse_id backfill を抑止
```
...
- 斤量・人気・調教師・騎手は OCR 非依存で決定的に取得する（#124 / ADR 0018）。
  - 斤量は CID 数字で読めるため `mutool -F stext.json` の座標索引から確定する（EdiF 復号不要。当初「斤量も EdiF」という前提は現行 PDF 形式では誤りだった）。
  - 人気は単勝オッズの昇順順位から算出する（EdiF の人気列を復号せずに決定的・正確）。同オッズは同順位。
  - 調教師・騎手も stext 座標索引から列単位で抽出する。
