...
- 各開催の OCR は `tesseract` を起動する。並列時は過剰なスレッド競合を避けるため OCR を 1 プロセス 1 スレッドに固定する（`OMP_THREAD_LIMIT=1` を自動設定）。
...
- **騎手は `mutool -F stext.json` の座標 + font サイズから列単位で抽出する**（テキスト抽出だと隣の馬主・調教師名や斤量が騎手列に連結するため）。騎手名は取り込み・検索の双方で正規化（全角英字/数字→半角、全角ピリオド `．`→`.`）され、`Ｃ．ルメール` でも `C.ルメール` でも `analyze jockey` が引ける。
  - 既存 DB に古い（汚染された）騎手値が残っている場合は、対象 PDF を再 ingest すると `jockey` 列が更新される。
- 馬名は完全一致で検索する（部分一致／カタカナ正規化等は未対応）。
