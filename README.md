...
- **docker ランタイム（colima 推奨）**: compose の `postgres` / `api` / `web` / `importer` サービスと
  ...
  起こす場合は `colima start` を先に実行し、その後「[DB（Postgres）](#dbpostgres)」の
  `docker compose up` を実行する。compose は `restart: unless-stopped` + healthcheck を持つため、
  colima が起動すれば既存コンテナは自己復旧する（compose の再実行は不要なことが多い）。
