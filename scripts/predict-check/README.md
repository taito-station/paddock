...
> 高くても EV は常に市場以下＝儲けは出ない。**儲けは「市場が間違っている所で市場と食い違う」ことからのみ
> 生まれる**。ゆえにモデルの本命的中率が市場（純市場 α=0.0）より低いこと自体は欠陥ではなく、評価軸は
> 的中率でなく ROI/EV（市場との乖離が当たるか）に置く。実際、純モデル（α=1.0）は単勝的中 12% と低いが
> 想定回収率 96.4% は純市場 73.1% を上回り、穴の過小評価（overlay）を突く弱い value シグナルを示す
> （ただし <100% で赤字・要改善。win 側の favorite-longshot 校正と value の検証は #305 / #272）。
