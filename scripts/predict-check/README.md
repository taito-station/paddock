...
> 単勝的中 12% と低いが想定回収率は市場と乖離した本命選択でむしろ優位に出る（純モデル 96.4%（母数 852）>
> 純市場 73.1%／現行 73.7%（母数 890）。母数差を含む横並び比較な点に留意）。ただしこれが真の value（市場の
> 過小評価を突く）か高オッズ的中の分散かは未検証で、<100% の赤字。加えて純モデルの低的中の一部は校正の弱さ
