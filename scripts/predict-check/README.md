...
> 食い違う」ことからのみ生まれる**。ゆえにモデルの本命的中率が市場（純市場 α=0.0）より低いこと自体が即
> 欠陥とは限らず、評価軸は的中率でなく ROI/EV（市場との乖離が当たるか）に置く。実際、純モデル（α=1.0）は
> 単勝的中 12% と低いが想定回収率は数値上はむしろ市場を上回る（純モデル 96.4%（母数 852）> 純市場 73.1%／
> 現行 73.7%（母数 890）。852 はトップ選好馬の単勝オッズが取れた非ランダム部分集合で、母数差・選択効果が
> 乗りうる横並び比較である点に留意）。ただしこれが真の value（市場の過小評価を突く）か高オッズ的中の分散
