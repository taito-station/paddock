...
依存: psql (PADDOCK_DB_URL), 標準ライブラリのみ。結果 HTML はローカルキャッシュ。
再実行可能。snapshot が増えたら母数が自動で増える。
"""
import os
import re
import sys
...
# netkeiba 場コード (JRA): 01札幌 02函館 03福島 04新潟 05東京 06中山 07中京 08京都 09阪神 10小倉
VENUE = {
    "sapporo": 1, "hakodate": 2, "fukushima": 3, "niigata": 4, "tokyo": 5,
    "nakayama": 6, "chukyo": 7, "kyoto": 8, "hanshin": 9, "kokura": 10,
}
...
def slug_to_netkeiba_id(slug):
    ...
    if venue not in VENUE:
        return None
    return f"{year}{VENUE[venue]:02d}{int(kai):02d}{int(day):02d}{int(r):02d}"
...
def parse_ts(s):
    # '2026-06-27T00:01:03.024586+00:00' -> epoch minutes (十分な精度で相対差だけ使う)
    m = re.match(r"(\d{4})-(\d\d)-(\d\d)T(\d\d):(\d\d):(\d\d)", s)
    y, mo, d, h, mi, se = map(int, m.groups())
    # ざっくり epoch 秒 (UTC, うるう秒無視・相対差用途)
    import calendar
    return calendar.timegm((y, mo, d, h, mi, se, 0, 0, 0))


def fetch_result_html(nk_id):
    path = os.path.join(CACHE, f"{nk_id}.html")
...
def parse_finish(html):
    """{umaban: finishing_position} を返す。除外/中止(非数値着順)は除く。"""
    res = {}
    for row in re.findall(r'<tr[^>]*class="[^"]*HorseList[^"]*"[\s\S]*?</tr>', html):
        rank_m = re.search(r'<td[^>]*class="[^"]*Result_Num[^"]*"[^>]*>([\s\S]*?)</td>', row)
        num_m = re.search(r'<td[^>]*class="Num Txt_C"[^>]*>([\s\S]*?)</td>', row)
        if not rank_m or not num_m:
            continue
        rank_txt = re.sub(r"<[^>]+>", "", rank_m.group(1)).strip()
        num_txt = re.sub(r"<[^>]+>", "", num_m.group(1)).strip()
        if not rank_txt.isdigit() or not num_txt.isdigit():
            continue
        res[int(num_txt)] = int(rank_txt)
    return res
...
def main():
    ...
    for rid, hs in by_race.items():
        ...
        if not nk_id:
            continue
        try:
            finish = parse_finish(fetch_result_html(nk_id))
        ...
        s_first = sum(1.0 / o_first for (_, o_first, _) in rec)
        used = False
        for num, o_first, o_last in rec:
            ...
            p_first = (1.0 / o_first) / s_first
            fin = finish[num]
            horses.append({
                "race": rid, "num": num, "o_first": o_first, "o_last": o_last,
                "drift": drift, "p_last": p_last, "p_first": p_first,
