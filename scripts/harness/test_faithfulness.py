...
class GateTest(unittest.TestCase):
    ...
    def test_gate_passes_with_zero_payout_races(self):
        # オッズ皆無の期間: backtest は「想定回収率 : —  (母数 0 レース)」を印字する。回収率の
        # 欠落を「パース退行」と誤判定せず、母数 0 として正当に通すこと（C1 回帰）。
        dump = self._write(
            self.HEADER + "R1\t1\t0.6\t0.7\t0.8\t1\t\n" + "R1\t2\t0.4\t0.5\t0.6\t2\t\n"
        )
        m = F.compute_metrics(F.load_dump(dump))
        self.assertEqual(m["payout_races"], 0)
        self.assertIsNone(m["payout_rate"])
