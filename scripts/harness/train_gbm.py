...
def load_gbm(path):
    """ダンプを [`train_pl.Race`] のリストに読む。`fund` を GBM 特徴量行列（15 列・NaN 保持）にする。

    `train_pl.evaluate` は `fund` を参照しない（winner/win_odds/baseline のみ）ので、`fund` に GBM の
    特徴量行列を入れて使い回せる。"""
    import csv
    ...
    for rid in order:
        rows = by_race[rid]
        n = len(rows)
        feat = np.full((n, len(GBM_FEATURES)), math.nan)
        win_odds = np.full(n, math.nan)
        baseline = np.full(n, math.nan)
        horse_nums, winner = [], None
        for i, row in enumerate(rows):
            horse_nums.append(int(row["horse_num"]))
            for j, col in enumerate(GBM_FEATURES):
                feat[i, j] = T._f(row[col])
            win_odds[i] = T._f(row["win_odds"])
            baseline[i] = T._f(row["model_win"])
            fp = row["finishing_position"]
            if fp not in ("", None) and int(fp) == 1 and winner is None:
                winner = i
        races.append(T.Race(rid, rows[0]["date"], horse_nums, feat, winner, win_odds, baseline))
    return races
...
def walk_forward(races, cutoffs, use_market, params):
    ...
    for k, cut in enumerate(cutoffs):
        ...
        y_tr = np.concatenate(
            [
                np.eye(len(r.horse_nums))[r.winner] if r.winner is not None else np.zeros(len(r.horse_nums))
                for r in train
            ]
        )
...
def main(argv=None):
    ...
    print(
        "\n注: flat ROI は「トップ選好馬の単勝 100 円」固定の参考値。木の特徴は rate6+signal3+starts6"
        "（市場版は +implied）。採否は複数指標で判断する。"
    )
    return 0
