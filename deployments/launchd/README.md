...
- **lock は UID スコープの固定パス**（`/tmp/paddock-keep-awake-$(id -u).lock.d`・#643）。`$TMPDIR` は使わない
  ...
  先回りされた場合は lock を取れず抑止が掛からないので、**所有者が自分でないときは警告を出す**
  ——ログに `lock パス … が自分の所有でない` が出ていたら、その日は抑止がゼロだったと読む。
- **旧 lock パス（uid 無し）からの移行**は `keep_awake.sh` / `uninstall.sh` の両方が 1 度だけ回収する。
  生きた `caffeinate` が記録されていれば引き継いで停止し、旧ディレクトリを片付ける。移行後は作られない。
