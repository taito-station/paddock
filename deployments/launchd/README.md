...
- **lock は UID スコープの固定パス**（`/tmp/paddock-keep-awake-$(id -u).lock.d`・#643）。`$TMPDIR` は使わない
  ...
  ——ログに `lock パス … が信用できない` が出ていたら、その日は抑止がゼロだったと読む
  （この経路は**非ゼロ終了**にしてあるので launchd 側のエラーとしても残る）。
...
- **ログ出力先（`WORKDIR`）は uid スコープ化していない**（既定 `$TMPDIR/paddock-keep-awake`）。#643 の
  スコープは lock のみで、ログ側は `prefetch_odds.sh` の lock（#651）と併せて別途扱う。
