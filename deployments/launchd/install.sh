...
# バックアップ鮮度監視（#490）・restore 検証（#474）の launchd エージェントを
# ~/Library/LaunchAgents/ に配置して有効化する。
# 各 plist テンプレートの __REPO_ROOT__（リポジトリパス）と
# __HOME__（$HOME・ログ出力先。backup-db / backup-staleness / verify-backup-restore が使用）
# を実値へ置換してから load する。
set -euo pipefail
...
# backup-staleness（鮮度監視）・verify-backup-restore（週次 restore 検証）の 5 エージェントを
# まとめて配置する。backup-db / backup-staleness / verify-backup-restore は常駐で、
# 開催日限定の uninstall では外さない
# （uninstall.sh は prefetch/keep-awake のみ除去する。#416 で二重規約を解消し install に統合）。
LABELS=(com.paddock.prefetch-odds com.paddock.keep-awake com.paddock.backup-db com.paddock.backup-staleness com.paddock.verify-backup-restore)
...
echo "      backup-db / backup-staleness / verify-backup-restore=$HOME/Library/Logs/paddock-backup.log"
echo "      （集約ログ。backup-db: 毎日 23:30 / staleness: 毎時 + 起動時 / verify: 毎週日曜 04:00）"
