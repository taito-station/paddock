...
# 通知はあくまで人間が判断するための参考値（decision-support・ADR 0055/0060）。張る/見送り/増額は人間が決め、軸は動かさない。
...
- **発走直前オッズの用途は "ズレ増額" のみ**。軸の馬が自モデル確率より美味しくオッズがズレた（＝過小人気）ときに、**軸を含む既存の買い目の金額を上げる**（＝増額）だけに直前オッズを使う。**点数（相手）は増やさない**——相手は各券種の既定幅（ワイド top3・馬連/3連複 top5）のまま。不利側にズレても軸は動かさない（レース全体の見送り＝そのレースを張らない、はあってよい）。
...
predict-watch は **decision-support（判断材料の提示）** であって自動 go/no-go ではない（ADR 0055/0060）。ツールが出す参考 ROI は張る/見送りと増額候補の検出の両方の判断材料に使い、**最終判断（張る/見送り/増額）は人間が決める**（ROI≥100% で張るの一次判断は「レース選択基準」の通り人間が適用）。軸は監視中も不変（上の「軸ロック」）。

- **唯一の正 = 最新サイクルの判定のみ**。前サイクル・朝の +EV リストは無効化する。
- **毎回の冒頭に 1 行で現況を明示**: 軸（不変）＋「🔶増額候補あり / ⚪妙味なし（据え置き）/ ⛔レースごと見送り」。曖昧な据え置きをしない。**ここで軸そのものは動かさない**（増額＝金額アップの可否だけを述べる。点数・相手は増やさない）。
- **ズレ警告必須**: 軸馬のオッズが前回から美味しくズレた（増額機会）／不利にズレた（増額見送り）を明言。レース全体の +EV↔−EV が反転したら見送り転換も明言（例:「函館10：朝+EV→直前−EVに転落、レースごと見送り」）。**軸 ◎ は黙って差し替えない・そもそも差し替えない**（差し替えは上記「新情報」時のみ・理由明示）。
