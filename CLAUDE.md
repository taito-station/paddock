...
- **探索規律 — 生読み前に mdq / cq 検索**: docs 内の答えを探すときは、まず
  `scripts/mdq search --q "..."`（BM25・ローカル・[.claude/skills/markdown-query/SKILL.md](.claude/skills/markdown-query/SKILL.md)）
  でヒットチャンクだけ取り、必要時のみ生ファイルへ。ソースコードの横断検索は
  `scripts/cq search --q "..."`（BM25 + symbol・[.claude/skills/code-query/SKILL.md](.claude/skills/code-query/SKILL.md)）。
