...
- 各点の金額は**券種予算内で 100 円単位の均等配分**（実装 `build_portfolio`/`distribute`）。券種予算を脚数で割り、賄える範囲で薄い脚にも同額を置く（**賄えない端数の脚は ¥0＝買わない**。脚ごとに ¥100 を必ず確保するわけではない）。総賭金は券種予算ちょうど（`/100*100`）。確率重み化＋脚ごと最低¥100 撤廃案は 71R で実 ROI 悪化のため棄却済み＝均等割りを維持（ADR 0046）。
- **印を打った馬は必ず買い目に絡める**（top5 まで広げる主因）。
- 上記は**実際に張る記録買い目**（`predict`→`build_portfolio`）の配分。`predict-check`／`predict-watch` の参考 ROI・買い目 slip（Python `live_ev.py`）は **model 確率重み＋最低¥100 の最大剰余法**で配分し配分方式が異なる（ADR 0064 が警告する second source。**張るのは均等割りの記録買い目が正**）。
