...
- **発走直前オッズの用途は "ズレ増額" のみ**。軸の馬が自モデル確率より美味しくオッズがズレた（＝過小人気）ときの **増額（相手は既存の top5 枠内・相手を広げない＝バックテスト 105R 準拠）** にだけ直前オッズを使う。不利側にズレても軸は動かさない（レース全体の見送り＝そのレースを張らない、はあってよい）。
...
predict-watch は **decision-support（判断材料の提示）** であって自動 go/no-go ではない（ADR 0055/0060）。ツールが出す参考 ROI は張る/見送りの一次ゲート（ROI≥100%）と増額候補の検出の両方の材料に使い、**最終判断（張る/見送り/増額）は人間が決める**。軸は監視中も不変（上の「軸ロック」）。

- **唯一の正 = 最新サイクルの判定のみ**。前サイクル・朝の +EV リストは無効化する。
- **毎回の冒頭に 1 行で現況を明示**: 軸（不変）＋「🔶増額候補あり / ⚪妙味なし（据え置き）/ ⛔レースごと見送り」。曖昧な据え置きをしない。**ここで軸そのものは動かさない**（増額・点数追加の可否だけを述べる）。
