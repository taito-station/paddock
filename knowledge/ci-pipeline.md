...
tags: [D21, D19, D17]
sources:
  - docs-original/616-docs-serving-checks.md
  - docs-original/636-fullwidth-after-var.md
...
- **移動コストが小さいことを実測で確認した**。`docs/adr` と `docs/docs-original` は同じ階層深さ（`docs/` 直下）なので、相対リンクはどの参照元からも「`adr` → `docs-original`」の 1 語置換で閉じる。ADR 本文が持つ兄弟相対リンク 8 件・`` 6 件・`../images/` 1 件・`../../deployments/` 2 件は**無変更で解決する**。ファイル名衝突も 0 件だった。
