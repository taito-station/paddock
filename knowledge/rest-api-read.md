...
> **旧記述の訂正**: #33 当時は `Interactor<R, P: PdfParser, F: PdfFetcher>` の 3 ジェネリクスで、read 経路でも `P`/`F` の具象型（no-op スタブ）を注入していた。「read 用途で P/F を型から外すリファクタは影響範囲が広いので別 Issue」としていたそれは **#453 で実施済み**で、PDF 系ユースケースは `PdfInteractor<R, P, F>` に分離された（[app-bootstrap.md](app-bootstrap.md)）。
...
- 308（メソッド保持リダイレクト）は GET のみの UI なので 307 でも 301 でも実質同等だが、永続的な正規化である意図を明示するために `web::redirect(..).permanent()` で 308 を返す。
