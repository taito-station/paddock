...
kind: knowledge
sources:
  - qa/QA-sources-coverage-checks-596.md
...
| D05 | ユースケースは CLAUDE.md の予想ワークフローと README.md が最も近いが、UC カタログの形では独立していない。個人プロジェクトの規模では不要 | ユーザー向けのシナリオカタログが必要になったとき |
...
| D16 | 個人プロジェクトで移行・ロールアウト計画を独立文書にする規模ではない。DB マイグレーション運用は ADR 0070 に記載 | マルチ環境デプロイや段階的ロールアウトが必要になったとき |
| D18 | LLM への入力統制は knowledge/README.md（2 層モデル・SoT 優先順位）と CLAUDE.md が担っており、独立したガバナンス文書にする規模ではない | Prompt の品質管理・入力統制を体系化する必要が出たとき |
...
[netkeiba-datasource.md](netkeiba-datasource.md) の「race_id 構築規則」）。**「現行 1」で充足ギャップの warning は消えるので、
...
| knowledge/backtest.md | [D24, D17, D19] |
| knowledge/betting-rule-history.md | [D24, D23] |
| knowledge/ev-kelly-bet-selection.md | [D23, D22] |
| knowledge/feature-resolution-diagnosis.md | [D24, D22] |
| knowledge/fetch-stage-split.md | [D19, D08, D15] |
| knowledge/jockey-recent-form.md | [D22, D24] |
| knowledge/learned-model-harness.md | [D24, D22, D19] |
| knowledge/live-ev-buy-view.md | [D11, D23, D10] |
| knowledge/netkeiba-datasource.md | [D10, D08, D09] |
| knowledge/predict-session.md | [D11, D19, D08] |
| knowledge/prediction-json.md | [D10, D08] |
| knowledge/prediction-search-api.md | [D10, D08, D19] |
| knowledge/probability-estimation.md | [D22, D19] |
| knowledge/race-result-ingestion.md | [D04, D10, D11] |
| knowledge/rest-api-read.md | [D10, D19, D09] |
| knowledge/session-write-api.md | [D10, D06] |
| knowledge/web-spa.md | [D11, D02, D10] |
...
stale も消える**。その穴を塞ぐため、**REQ 表の `出典` 列が名指しした `docs-original/`
...
**`qa/` は対象外。** どの `sources` からも参照されていない qa ファイルがあっても鳴らない。
...
### #681: D05/D16/D18 を n/a に閉じる (2026-09-13) — 採用

#### コンテキスト

HVE 導入時に D01〜D24 の全クラスを active として引き継いだが、D05（ユースケース・シナリオカタログ）・
D16（移行・導入・ロールアウト計画書）・D18（Prompt ガバナンス・入力統制パック）は
paddock の規模・運用形態では独立文書にする実利がない。check-doc-classes.py が毎回
「充足ギャップ」を警告していた。

#### 決定

3 クラスを n/a に変更し、充足ギャップ警告を解消する。
...
- **空の placeholder 文書を作る**: 充足ギャップ警告は消えるが、中身のない文書が
  形骸化する。n/a 宣言の方が意図が明確
- **check-doc-classes.py の警告を suppress する**: 警告機能自体は有用。個別の
  クラスを n/a にする方が適切

#### 影響

- `python3 scripts/check-doc-classes.py` の充足ギャップ警告が 3 件 → 0 件に
