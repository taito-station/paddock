...
- 反映先: ADR 0085 / knowledge/predict-session.md
...
- 観測/根拠: SPA は「終了」を**結果確定**で判定し、`post_time` 経過だが未確定のレースは未発走側に
  残す（#381・`knowledge/web-spa.md`）。一方 #587 の要件は「実行時刻を基準に発走済みを明示。
...
- 回答: **確定。ADR 起票 + `knowledge/predict-session.md` への写し**（ユーザー確認済み）。
