...
> 質問票+回答（[qa/README.md](README.md)）。一次資料は issue #409（全体レビュー 2026-07-15 由来）。
> 関連確定知: [ADR 0014](../docs-original/0014-none-baseline-exclusion.md)（predict・backtest は同じ `build_factors`/`estimate_probabilities` を共有）、
> [ADR 0055](../docs-original/0055-ev-layer-separation-circular-break.md)（`predict_race_views` は factor 収集 1 回）。
...
- 反映先: `src/use-case/src/interactor/race/predict.rs`、knowledge（scoring 経路の factor 収集構造）。
  → 蒸留完了: [`knowledge/scoring-factor-collection.md`](../knowledge/scoring-factor-collection.md)（#487）。
