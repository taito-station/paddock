...
- 観測/根拠: main `5ae6466` 時点で、`check-doc-classes.py` のパーサ
  ...
  出典セルのリポ内リンクは **37 本すべてが `docs-original/` の 4 桁 ADR**（非 ADR は 0 本）。
...
- 観測/根拠: 選択肢は (a) スクリプト内の定数リスト、(b) `knowledge/doc-classes.md` の宣言表。
...
- 反映先: ADR 0083 理由 / `knowledge/doc-classes.md`
...
## Q4: #597 の突合対象を「ADR」に限るか、`docs-original/` 配下に広げるか
...
- 回答: **確定。`docs-original/` 配下に広げる（4 桁 ADR に限定しない）。** issue の字面は
