...
セルフレビュー 5 巡を経た**最終形**での実測。巡ごとに判明した欠陥は決定ログ側に記録があるが、
特にこの節に関わるのは「起動注記が測っていない閾値について ADR 0076 を引いていた」（1 巡目）と
「同じバナー内で閾値の丸め・マーク案内が食い違っていた」（4〜5 巡目）の 2 点。

### osascript の呼び出し

- 終了ステータスは成否を弁別する: 壊れたスクリプトで `1`、正しい呼び出しで `0`。
- 本文に `"` と `\` を混ぜた通知が成功で返る（argv 渡しの検査を兼ねた `sends_real_notification`
  手動テスト。デッドライン＋kill の経路をそのまま通る）。
- **ただし exit 0 は「osascript の起動と実行に成功した」まで**で、バナーが実際に表示されたことは
  保証しない（通知許可なし・集中モードでも 0 を返す）。そこは次の開催日に実地確認する。

### 起動バナー（実バイナリ + 実 DB・23:05・全レース発走済みのため対象 0 レース）

```
$ paddock-predict-watch --date 2026-08-23 --once 
── 参考ROI の読み方: 🔶（≥100%）は 182R / 839 スイープの実測で到達 0 件。判定ROI に実現ROI の選別力は無い（ADR 0076）。
   🔍（≥70%）は結果照合のための目印で、張り推奨ではない。
   張る/見送りは参考ROIでは決めない——手動のハンデ精査と執行の規律（軸ロック＋ズレ増額）で決める（ADR 0055/0060）。
── macOS 通知: 有効・発火は参考ROI ≥ 100%（--notify-roi 未指定＝--roi-gate 追従）。表示ゲートは別物で 🔶 ≥100% / 🔍 ≥70%（ベルは鳴らさない）。
...
── 23:05 スイープ: 対象 0 レース（窓 40分 / 🔶買い妙味≥100% ・ 🔍検証候補≥70%・判定は手動精査）
── 監視終了: 発走前のレースが残っていません。

$ paddock-predict-watch --date 2026-08-23 --once --roi-gate 0.7
── 参考ROI の読み方: 🔶（≥70%）は買う閾値の表示マーク。判定ROI に実現ROI の選別力は無い（ADR 0076。到達 0 件と実測したのは ≥100% で、この設定はそれより低い）。
   張る/見送りは参考ROIでは決めない——手動のハンデ精査と執行の規律（軸ロック＋ズレ増額）で決める（ADR 0055/0060）。
── macOS 通知: 有効・発火は参考ROI ≥ 70%（--notify-roi 未指定＝--roi-gate 追従）。表示ゲートは別物で 🔶 ≥70%（ベルは鳴らさない）。
   同一レースは前回通知時から +10pt 上振れしたときだけ再通知。配送はベストエフォート（表示セッション依存）で、一次情報はログの 🔔行。鳴っても go シグナルではありません（ADR 0079）。
── 23:05 スイープ: 対象 0 レース（窓 40分 / 🔶買い妙味≥70%・判定は手動精査）
── 監視終了: 発走前のレースが残っていません。
...
── 参考ROI の読み方: 🔶（≥100%）は 182R / 839 スイープの実測で到達 0 件。判定ROI に実現ROI の選別力は無い（ADR 0076）。
   🔍（≥70%）は結果照合のための目印で、張り推奨ではない。
   張る/見送りは参考ROIでは決めない——手動のハンデ精査と執行の規律（軸ロック＋ズレ増額）で決める（ADR 0055/0060）。
── macOS 通知: 有効・発火は参考ROI ≥ 50%（--notify-roi 指定）。表示ゲートは別物で 🔶 ≥100% / 🔍 ≥70%（ベルは鳴らさない）。
...
- `--roi-gate 0.7` では **ADR 0076 の「到達 0 件」を断言しない**（測ったのは ≥100%）。
- 同じ設定では 🔍 帯が構造的に空（`notify_gate == roi_gate`）なので、**🔍 をどの行でも案内しない**。
- 閾値表記は全行で同じ `pct()` を通しており、丸めでゲートが食い違わない。
