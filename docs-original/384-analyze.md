...
> RO 生素材（[docs-original/README.md](README.md)）。書き換えず、ここから qa→knowledge を蒸留する。
...
- doc（**stale**の可能性・下記実測参照）:
  - `knowledge/rest-api-read.md` §4 分析統計:
    > 名前あいまい検索（部分一致・カタカナ正規化, #50）は本 Issue では扱わない。完全一致でドメ…（略）
  - `knowledge/web-spa.md` §[4] 分析ビュー:
    > 馬名・騎手名検索は #50（部分一致・カタカナ正規化）に追従する。
  - `knowledge/prediction-search-api.md`「馬名の正規化（#50 流用）」に正規化の設計あり（横断検索 API 側）。
