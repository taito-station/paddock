...
4. **取得失敗は観測しない。** `fetch_one_exotic` は失敗した券種を呼び出し側へ伝え
   （`FetchedExoticOdds::failed`）、失敗した券種は未発売マークを作らない。
   失敗は「分からない」であって「売っていない」ではなく、**次回そのまま取り直させる必要がある**
...
6. **`RaceOdds::is_complete()` の意味は変えない。** 「priced な行が全券種そろっているか」のまま
...
7. **priced が取れた券種のマークは同一トランザクションで削除する。** 発売が始まったのに
   「未発売」の観測が残ると、次にその券種が一過性失敗で欠けたとき誤って cache-hit してしまう。

8. **観測は `predict-watch` 経路（`refresh_race_odds`）でも記録する。** 監視側は cache を見ないが、
...
- **`FetchedExoticOdds::default()` をそのまま `assemble_netkeiba` に渡すと全券種が未発売と
  誤判定される**（`failed` が空＝「取得成功して 0 行」に見える）。観測していない経路は
  `all_exotic_failed()` を使う。回帰テスト
  `win_place_only_scrape_observes_no_unpriced_bet_type` で固定した。
