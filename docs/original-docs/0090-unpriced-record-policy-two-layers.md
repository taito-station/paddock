...
- **表現を過去分に遡って揃えない**（番兵行を観測表形式へ変換して移行する等をしない）のは、
  歴史の書き換えに相当し、変換そのものが新しいバグ面を作るため。層の境界は 2 本の ADR の
  マージ時点で機械的に引ける——**ADR 0086（2026-08-16）以前 = snapshots の生値 /
  ADR 0089（2026-08-19）以後 = 観測表**。間の約 3 日はどちらの表現でも記録されない空白帯だが、
  JRA 開催の無い平日に当たるため実データの欠落はほぼ無い。
