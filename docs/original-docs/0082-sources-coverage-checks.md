...
6. **`sources` と例外表のパスは正規形（`docs/...`）だけを許す。** `./docs/...` のような
   ...
   **形式を 1 つに強制する**方を採る。導入時点で `sources` 214 行・例外表 1 行すべて正規形。
...
調査所見の置き場で、蒸留先を持つとは限らない）。この結果、導入時点で
`601-axis-flip-in-predict-watch.md` を含む非 ADR 一次資料 1 本と `docs/qa/` の 4 本が
どの `sources` からも参照されていないが、これらは検査対象外なので鳴らない。
...
- **不変**: 既存の 10 項目の検査と `scripts/bump-distilled-sha.py` がパースする STALE 行の文言は
