# 0076. 用語集は正本を指す索引に徹し、`sources` は確定知層まで許すが `CLAUDE.md` は入れない
...
3. **`CLAUDE.md` は `sources` に入れない。** 正本列で指すに留める。理由は 2 つ:
   ...
   同じ理由でソースコードと文書運用の規約文書（`README.md` / `doc-classes.md`）も `sources` に入れない。
...
- `sources` に可変な確定知層 9 本を取るため、それらの本文が動くたびに用語集の
  `distilled_from_sha` 追従が要る。**これはコストではなく仕様**（見直しの強制が狙い）。
- 用語集の本文リンク（40 本超）と節名参照は機械検査の対象外。REQ 表を持たない文書なので
