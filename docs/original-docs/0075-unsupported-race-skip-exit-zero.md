...
1. `netkeiba_scraper::Error::Unsupported(String)` を新設し、`parse_card` の障害判定はこれを返す。
   未知の馬場記号・`RaceData01` 欠落は従来どおり `Parse`（**「対応外」を広げない**）。
...
加えて決定的なのは消費側の実装で、`scripts/predict-check/refresh_ev.sh` は **exit≠0 を一律 FAIL 扱い**
して「古いオッズ警告」を出す。専用 exit code を作ると対応外レースが取り込み失敗に計上され、
**本 issue の目的（実障害だけを FAIL にする）を達成できない**。exit 0 なら同スクリプトは無改修で正しく
動き、呼び出し側は「exit≠0 = 本物の失敗」と単純に扱える。
