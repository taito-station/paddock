...
セルフレビュー 2 巡を経た最終形での実測。1 巡目では「`--roi-gate` を下げると、測っていない閾値に
ついて ADR 0076 の『通過 0 件』を宣言してしまう」ことが判明して分岐を組み直し、2 巡目では
「配送失敗でも抑制状態を消費する」「`spawn_blocking` は中断できず子プロセスも残る」ことが判明した。

### osascript の呼び出し

- 終了ステータスは成否を弁別する: 壊れたスクリプトで `1`、正しい呼び出しで `0`。
- 本文に `"` と `\` を混ぜた通知（`函館10R 15:35 テスト"賞\  ・ 参考ROI 123.4% ・ 軸7`）が
  成功で返る（argv 渡しの検査を兼ねた `sends_real_notification` 手動テスト・デッドライン経路を通る）。

### 起動注記（実バイナリ + 実 DB・22:19・全レース発走済みのため対象 0 レース）
...
── macOS 通知: 有効・発火は参考ROI ≥ 100%（--notify-roi 未指定＝--roi-gate 追従）。表示ゲートは別物で 🔶 ≥100% / 🔍 ≥70%（どちらもベルは鳴らさない）。
   ...
   同一レースは前回通知時から +10pt 上振れしたときだけ再通知。配送はベストエフォート（表示セッション依存）で、一次情報はログの 🔔 行。鳴っても go シグナルではありません（ADR 0079）。
...
```
$ paddock-predict-watch --date 2026-08-23 --once --notify-roi nan
Error: --notify-roi（NaN）は 0 以上の有限値で指定してください（NaN は比較が常に false になり、通知が無言で鳴らなくなります）。

$ paddock-predict-watch --date 2026-08-23 --once --roi-gate nan
Error: --roi-gate（NaN）は 0 以上の有限値で指定してください（NaN/∞ は表示マークと通知の判定を無言で壊します）。
```

`--roi-gate` 自体の検証は本 PR で足した。無いと未指定の `--notify-roi` が NaN を継承し、
`roi < NaN` が常に false になるため **Due 全レースが初回スイープで一斉発火**する。

**未実施**: 実際にゲートを通過したレースで通知バナーが出るところまでの end-to-end は、
2026-08-23 の作業時点で当日の全レースが発走済みのため確認できていない。次の開催日に
