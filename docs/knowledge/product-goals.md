...
| #706 | ズレ増額の符号反転（直前オッズ低下馬への増額） | 15 分粒度の実測 388R で文献と逆符号（低下馬はむしろ低回収・δ̂ CI 負） |
...
### #706: 直前オッズ変化の方向 × 実現回収率（ズレ増額の符号検証）— 文献と逆符号を確認・現行維持 (2026-09-23) — 棄却（符号反転案）

#### コンテキスト

Hanyu, Ishii, Otani & Teramoto (2025, arXiv:2509.14645・JRA 2004〜2023・約 63,000R・5 分刻み)
は「同一最終オッズなら**直前 5 分でオッズが下がった馬**の実現回収率が有意に高い」
（5 分窓 −0.356、15 分窓では −0.105 に減衰）を示し、現行ズレ増額（CLAUDE.md「軸ロックと
ズレ増額」/ ADR 0060 = 過小人気側=オッズ上昇側にズレたら増額）と**逆符号**の可能性が
あった。ADR 0067 が明文化した再検証トリガ（「数百 R 規模に達したら再走して再評価」）を
`race_odds_snapshots` 540R で発動し、ADR 0067 との差分 3 点——(1) 評価が logloss 残差で
なく**実現回収率**、(2) 窓が朝→最終でなく**発走直前 lag 基準**、(3) 対象が予測シグナル
でなく**増額トリガーの符号**（執行側）——を測った。判定基準は issue #706 コメントで
実装前に事前コミット（改訂案提示 = δ̂ 95% CI が 0 を跨がず正〔Δ>0=オッズ低下馬が有利〕
かつ 層別加重差が全体・低オッズ層とも正。それ以外は現行維持）。

#### 決定

**現行ズレ増額の符号は維持する（符号反転案は棄却）**。測定はむしろ現行の向きと整合する
逆符号——同一確定オッズ条件付きで「直前 ~15 分にオッズが**下がった**馬」の回収が**悪い**
——を示した。

#### 理由

`scripts/predict-check/zure_sign_probe.py`（stdlib・psql・netkeiba キャッシュは
late_money_probe と共有）。t_last = 発走前（lag≥0・JST 日付一致）最後の win snapshot、
t_prev = 10 分以上前の最近傍。Δ = ln(odds_prev) − ln(odds_last)（>0 = オッズ低下）。
回収・条件付けはともに**確定単勝オッズ**（`results` を優先し、無いレースは netkeiba 結果
ページの単勝オッズ列）。確定オッズは snapshot と同じ `odds_guard` を通す。整合検査として
実効レースすべてで「勝ち馬の確定オッズ×100 = netkeiba 単勝払戻」を照合した（results 経路
118 点・netkeiba パーサ経路 270 点＝実効 388R の勝ち馬 1 頭ずつ・不一致 0）。この検査で
1 着同着レース（払戻が按分され確定オッズ×100 にならない）を 1R 検出し、#703 の
`winner_races` と同じく除外した。

- 母集団 540R → **実効 388R / 4,944 頭**（除外: 発走前 snapshot なし 75R / t_prev なし
  76R / 1 着同着 1R。頭単位では結果側に馬番の無い取消等 203 頭）。実効窓は中央 13.9 分
  （最小 10.0 / 最大 381.2）。Δ=0 は 2.9%。
- **δ̂ = −1.128 [95% CI −1.911, −0.290]**（won ~ a + b·ln(確定オッズ) + δ·Δ、
  レース単位 bootstrap n=2000・seed=42・percentile・非収束 0）——CI が 0 を跨がず**負**
  ＝文献方向の逆。
- 層別（ln(確定オッズ) 五分位 × Δ 3 群）も同方向: 加重差（Δ>0 − Δ≤0）は
  **全体 −27.3 円/100 円・低オッズ層 Q1-2 でも −13.0 円**（Δ>0 vs Δ<0 の平均回収:
  Q2 49.3 vs 75.8 / Q4 45.4 vs 76.5 / Q5 0.0 vs 64.6）。
- 併走の `late_money_probe.py` 再走（330R / 4,355 頭・94R 時代の約 3.5 倍）: 生相関
  Spearman(drift, finish) = −0.362（94R の −0.378 と同水準の交絡）、終値水準を揃えた
  五分位残差は非単調のまま、best drift 係数 **b = −0.10・Δlogloss +0.0001** ＝
  「クロージングが吸収済み」の ADR 0067 棄却を 540R 規模でも追認（符号はわずかに負で
  本測定と整合）。

解釈（仮説として記録・確定知ではない）: 15 分粒度で見える「締まり」は informed money
（Hanyu の 5 分効果）ではなく、過剰反応の巻き戻し側が支配的とみられる。5 分効果と 15 分
での逆転は矛盾しない（Hanyu 自身が窓拡大で急減衰を報告している）。

#### 却下した代替案

- **符号反転（オッズ低下馬への増額）**: 実測が逆方向（δ̂ CI 負・層別も負）で棄却。
- **t_last snapshot オッズでの条件付け**: t_last→確定間ドリフトが文献方向を機械的に
  打ち消すバイアスになるため、確定オッズ条件付けに設計変更（Plan 敵対的レビュー Major-1）。
- **5 分窓の直接再現**: 収集が実質 15 分粒度（1〜16 分不定）のため原理的に不可。

#### 影響

- CLAUDE.md「軸ロックとズレ増額」・REQ-D01-003・glossary の「ズレ増額」行は**すべて不変**
  （改訂 PR 不要）。
- ADR 0067 の reject-for-now 再検証条件（数百 R）は本測定で**履行済み**。再々検証の条件:
  **5 分粒度の snapshot が数百 R 蓄積された場合のみ**（odds-collect は 15 分毎なので現状の
  収集では発生しない。predict-watch 併用日の直前詰まり〔1〜6 分刻み〕が蓄積すれば部分的に
  近づく）。
- 限定条件: 実効 388R は Hanyu の 63,000R に対し検出力が大きく劣る（ただし本測定の CI は
  0 を跨いでおらず「逆方向に有意」）/ **窓に上限が無い**——t_prev は「10 分以上前の最近傍」
  で、「直前 ~15 分」は実効窓の中央値の意。最大 381 分の長窓（朝→直前の drift に近い）が
  混ざった推定であり、窓を絞った感度分析は事前コミット外の別測定として #713 で行う /
  単勝回収は増額対象券種（ワイド/馬連/3連複）の proxy / 94R⊂540R で ADR 0067 の母集団と
  非独立。
- 再測定手順は `zure_sign_probe.py` として残る（netkeiba キャッシュ共有・整合検査・
  除外内訳の出力込み）。棄却記録は betting-rule-history.md ⑬ にも要約を追記。
