...
| REQ-D01-006 | 手動ハンデ精査を伴う実運用セッションの精度（観測値: 本命単勝 43.5% / 複勝 65.2% / 勝ち馬 Top5 包含 87%）がバックテスト水準を上回り続ける | 実運用セッションの `bet_records` と結果照合を 200R 以上まで貯めて再集計する（現状の観測は 2026-06-13 の 1 開催日規模で母数が足りない） | セッション観測 2026-06-13（リポジトリ外） | Tentative |
