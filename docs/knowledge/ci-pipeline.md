...
- **evil merge は不可視ではない**（ADR 0084。ADR 0081 の「既知の限界 (1)」の訂正）。
  ...
  **この「食い違いが無い」は計測で確かめてある**——`status is None` の分岐に到達するかを
  計装して数えると、実リポジトリ全体（`sources` 110 パス・全履歴）でも回帰テスト 183 ケースでも
  **0 回**。当該分岐は防御的で実際には通らない（`git log` にオプションを足すと前提が崩れるので
  分岐自体は残してある）。
