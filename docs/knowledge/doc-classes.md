...
| D05 | paddock は個人プロジェクトで運用手順は CLAUDE.md + keiba-start スキルに内包 | チーム運用開始時 |
...
| D16 | 開発環境は CLAUDE.md + docker-compose.yml で記述済み | マルチ環境デプロイ開始時 |
| D18 | DB マイグレーション手順は CLAUDE.md に記載、大規模移行予定なし | 大規模システム移行時 |
...
HVE 導入時に D01〜D24 の全クラスを active として引き継いだが、D05（運用手順）・
D16（環境構成）・D18（移行計画）は paddock の規模・運用形態では独立文書にする
実利がない。check-doc-classes.py が毎回「充足ギャップ」を警告していた。
...
- D05: 運用手順は CLAUDE.md と keiba-start スキルに内包されており、分離すると二重管理になる
- D16: 開発環境は CLAUDE.md + docker-compose.yml で記述済み
- D18: DB マイグレーション手順は CLAUDE.md に記載。大規模移行の予定がない
