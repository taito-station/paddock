...
- **`要件` と `出典` は空にできない**。出典に書くリポジトリ内リンクは実在すること（`docs/` の外や
  絶対パスは不可）——由来を辿れない要件は根拠を確認できない。
...
- マーカーの対応と書式、**ブロック外に取り残された REQ 行**、コードフェンスの閉じ忘れ
  （いずれも「表が丸ごと無検査になる」経路なので塞いである）
- 見出し行・区切り行・列数、REQ-ID の形式、ID のクラス部とブロックのクラスの一致
- 番号の重複、`status` の値域、`要件` / `出典` の非空、Confirmed の検証手段、リンク先の実在

一方、**次の 2 つは機械検査できない**ので人手の規律に残る:
