...
6. **sources 追従**: knowledge の `sources` に列挙されたファイルを**内容ごと**変更する PR は、参照元
   ...
   - **例外 1d: `uses:` のピン留め SHA 更新だけの差分は「内容変更」と見なさない**（ADR 0081）。
     ...
     構造的に効かない。機械検査は「行数が同じ」かつ「差分行がすべて `uses:` のピン行」かつ
     「owner/repo が同一」のときだけ遡る。**変わってよいのは 40 hex と末尾のバージョン注記
     （`# v2` → `# v2.9.2`。dependabot がここも書き換える）だけ**で、action の差し替え・
     タグへの緩和・行の増減はいずれも内容変更のまま。
