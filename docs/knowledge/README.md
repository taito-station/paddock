...
- **機械検査**: 上記スクリプトが CI（`adr` ジョブ）と pre-push で走る。クラスの整合・`tags` の一致・
  ...
  の 4 つ目に注意——`sources` から行を消せば stale も消える）。**`doc-classes.md` のマーカー
  欠落だけは `--warn-only` でも 1 で落ちる**（表の範囲を切り出せず、検査そのものが成立しないため）。
...
- **インライン形式以外のリンク**。`[label]: path` の参照形式定義と HTML の `<a href>` は
  ...
  見なさないので、その中のリンク様文字列は実データとして検査される。
...
- **索引の網羅性（doc-classes.md の割当索引を除く）**。下記「REQ 表のある文書（索引）」と
  `doc-classes.md` の「充足ギャップ」表は手書きで、実態との突合は無い（割当索引だけは #604 で
  機械検査に載せた）。
