...
刻み幅 30 秒の根拠: 最短スイープ間隔 1 分でも 2 ティックは刻める粒度（既定なら predict-watch
5 分＝10 ティック / odds-collect 15 分＝30 ティック）。

**「最大 30 秒で再開」が成り立つのは本格復帰してから**。`tokio::time::sleep` は単調時計基準で
macOS ではスリープ中に進まないため、7 秒 DarkWake を繰り返すだけの Standby では 30 秒ぶんの
awake 時間が貯まるまで再開しない（8/1 の一次資料がこの状態）。蓋を開けた時点からは 30 秒以内に
再開し、発走直前 EV の判断粒度（分オーダー）には響かない。
