...
**天井は coverage でなく factor 冗長性**（ADR 0058 訂正で確定）。gated 4,594R を「1レース内の馬履歴 factor（horse_surface/distance/track_condition）カバー率」で層別すると、model AUC はフラット（0.65-0.685）で、フル装備の 100% 層(0.685) は履歴ゼロの 0% 層(0.677) を +0.008 しか上回らない＝馬履歴 factor は常在の course_gate/jockey/trainer に冗長（`/tmp/pa/coverage_strata.py`）。※ ADR 0058 初版の「coverage cap 19.5%」は sire を `results.horse_id`(20%) で join したアーティファクトで馬 factor 一般の天井ではない（実 coverage ~60-71%）＝同 ADR 訂正節参照。全 runner 履歴の大量 fetch arc は不要。次に動かすなら公開データ外の情報が要る（ADR 0027）。
