...
| POST | `/api/sessions/{date}/races/{race_id}/odds:refresh` | オッズをライブ取得して保存（#51） | 200 / 404 / 502 |
| POST | `/api/sessions/{date}/results:refresh` | 確定結果を取得して払戻を自動補完（#40） | 200 / 404 / 502 |
...
- `OddsInteractor`（live scrape, ADR 0005）でオッズを取得し `race_odds` に保存（#51）。取得不可は 502。セッション未作成は 404。レスポンスは保存したオッズの要約。

### POST /api/sessions/{date}/results:refresh（#40）

- `SettleInteractor::settle_session(date)`（#40）で確定結果を取得し、per-bet 払戻を自動補完・残高/累計を再計算して保存。レスポンスは更新後サマリ。取得不可は 502、未作成は 404。
...
→ `use_case::Error` に **`Conflict(String)`** を追加し、rest-controller 側で 409 に写像する。残高超過・budget 不正は `InvalidArgument`（400）。外部取得失敗（odds/results refresh）は既存 `Fetch`/`Timeout` を 502 に写像する。
...
rest-controller の `Error` enum にも `Conflict` を追加し、`ErrorBody`（`{ "error": { code, message } }`）で返す。500 は #33 同様に内部詳細を伏せてログのみに出す。
