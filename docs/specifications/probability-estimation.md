...
|---------|-----|----------|
| `course_stats` | venue × distance × surface | **枠順グループ別** 勝率 / 連対率 / 複勝率（`course_gate_rate`） |
| `horse_stats` | horse_name | **芝ダ別**勝率（`horse_surface_rate`）・**距離帯別**勝率（`horse_distance_rate`） |
| `jockey_stats` | jockey_name (任意) | **芝ダ別**勝率（`jockey_surface_rate`） |
...
raw_score =
  ...
  + 1.0 × jockey_surface_rate       // 騎手の芝ダ実績（騎手なし時は 0 として加算）
```

> 注: 「合計重み」は正規化の除数ではない。正規化はレース内の全馬スコア合計で行う（ステップ 3）。騎手なし馬はスコアが低くなるが、重み合計による補正は行わない。
