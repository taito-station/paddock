...
| `find_recent_runs` | horse_name × cutoff 日 | **直近 N 走（N=1〜3）** から算出する前走フォームトレンド（`recent_form: Option<f64>`, #31/#220）。重みは [1.0, 0.5, 0.25] 固定の加重平均。本番 predict は N=1 固定、`--trend-n` は `paddock-analyze backtest` スイープ専用（#220, ADR-0035）。cutoff より前のみ（リーク防止） |
