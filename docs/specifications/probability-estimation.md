...
**フォールバック条件:**
- 個別馬のスコアが 0（スタッツ未蓄積等）の場合: その馬のスコアは 0 のまま正規化に含める。他の馬のスコアが正であれば正規化結果も正になる
- **全出走馬のスコア合計が 0**（出走馬全員のスタッツが未蓄積）の場合のみ均等確率（1/頭数）にフォールバックする
...
**影響範囲（全件変更が必要）:**
- `src/interface/rdb-gateway/src/repositories/horse_stats.rs`: 5 クエリパターン（overall / by_surface / by_distance_band / by_track_condition / by_popularity_band / by_gate）
...
pub struct HorseFactors {
    pub course_gate_rate: f64,      // course_stats の枠順グループ勝率
    pub horse_surface_rate: f64,    // horse_stats の芝ダ勝率
    pub horse_distance_rate: f64,   // horse_stats の距離帯勝率
    pub jockey_surface_rate: Option<f64>, // jockey_stats の芝ダ勝率（騎手なし時 None）
}
```

（place / show 確率のための同等フィールドも同様の構造を持つ）
...
2. 各 HorseEntry に対してスタッツを逐次取得（`join_all` 等による並列化は実装フェーズで選択）
