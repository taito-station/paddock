...
変数展開は zsh で単語分割されず動かないため（#594 で実際に両方を踏んだ）。
...
| REQ-D22-008 | 相性 factor（騎手×場 / 騎手×距離 / 騎手×馬 / 馬×場）は production 非組込（重み 0）。measure-first で lift を測ってから採否を決める | `for w in 0.0 0.5 1.0; do paddock-analyze backtest --from 2025-01-05 --to 2026-06-14 --blend-alpha 0.2 --shrinkage-m 10 --win-power 1.25 --place-show-power 2.0 --impute-missing-factors --jockey-venue-weight "$w" --jockey-distance-weight "$w" --jockey-horse-combo-weight "$w" --horse-venue-weight "$w"; done` で cheap screen。**本命 top1 が baseline を lift 閾値ぶん上回ること**が採用条件（劣化しないだけでは採らない。閾値と最終的な掃引値は #350 で確定する。ADR 0061 は同型案件で純 top1 +0.010〜0.015 のゲートを置いた） | [#350](https://github.com/taito-station/paddock/issues/350)（measure-first で保留中。採否の ADR は未起票） | Tentative |
