...
raw_score =
    ...
    ) / Σ(present weights)           // 例: 騎手・調教師・馬場・前走あり=7.25、すべてなし=4
```

> 注1: 騎手未登録馬（`jockey_surface = None`）・調教師なし／実績なし馬（`trainer_surface = None`）・
> 前走なし馬（`recent_form = None`）・馬場状態の実績がない馬（`horse_track_condition = None`）はその項と
> 重みを**母数から除外**するため、欠落項で不当に減点されない（ADR 0007/0008）。これらの項は「平均からの
> 差分」としてのみ効く。全馬が同条件のときは定数除算となり相対順位は不変。
