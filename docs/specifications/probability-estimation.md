...
### #284: (m,α,γ) 確定チューニング — 測定完了・定数変更は #218 待ちで保留 (2026-09-23) — 保留

#### コンテキスト

ADR 0045 の暫定結論（71R・過学習留保）を確定させる掃引。#282 の Rust 3 軸ツールは作らず、
dump 3 本（m∈{5,10,20} 焼き込み・同一 SHA・行数/レース集合一致を確認）× α∈{0, 0.05, 0.1,
0.2, 0.3, 0.5, 1.0} × γ∈{1.0, 1.25, 1.5} をオフライン厳密再構成（`scripts/predict-check/
mag_sweep.py`。u ∝ est^γ0 の逆変換・#703 Phase 3 の手法）で評価した。忠実性アンカー
（(0.2, 1.25) の再構成 = dump の model_win 列・全行 max|Δ|<1e-9）で等価性を担保。
評価軸は Spearman/ROI ゲート（issue 起票時）ではなく #703 計測器の ΔR²/Brier/top1 に置換
——根拠は (a) 製品側が参考 ROI をゲートに使わない（REQ-D01-001 改訂）、(b) ADR 0045 の
71R 掃引で全 (α,γ) がゲートを回復できなかったこと。

計測で見つかった副産物: dump の `win_odds` 列（馬単位 fallback）と blend が実際に使う
市場マップ（レース単位切替 + as-of）が乖離するレースが実在する（race_odds の win 行が
レース後に再取得され、as-of を通る行が取消馬の断片のみ → blend 実質 no-op なのに列には
PDF オッズが載る。実例 2026-2-kokura-8-7R。eval 窓 9R・0.6% / fit 窓 0R）。このクラスは
忠実性アンカーで検出し全系統・全候補から一律除外した（1% 超で停止するガード付き）。

#### 決定

**本番定数（m=10 / α=0.2 / γ=1.25）は変更しない（保留）**。測定上の採用ゲートは通過した
（下記）が、本測定はクローズオッズ基準であり、本番 predict が α を適用する朝〜直前オッズへの
外挿は未測定のため、**#218（live オッズでの α 再校正・`race_odds_snapshots` 540R）の結果と
セットで最終判断**する（PO 判断・2026-09-23）。

#### 理由

- fit 窓（2025 年・2,721R 中アンカー除外 0）: **63 格子点のどれも market を上回れない**
  （最大 ΔR² = 0.0000 = α=0 の market 恒等）。α を増やすほど単調悪化（α=1.0 で −0.21〜−0.23）。
  m 軸は α>0 でも差が微小（REQ-D22-002 の既知実測と整合）。
- 事前コミットのタイブレーク（現行距離最小）で候補 = **(m=10, α=0.0, γ=1.0) = 市場そのもの**。
- eval 窓（1,473R 中アンカー除外 9）:
  (a) paired ΔR²(候補−現行) = **+0.0044 [95% CI +0.0020, +0.0067]** → PASS
  (b) market 恒等（α=0 **かつ γ=1.0**）につき自動成立 → PASS。事前コミット文言の
  「α=0 なら自動成立」には γ 条件が欠けていた（α=0 でも γ≠1.0 は normalize(q^γ)≠q）
  ——実装（`is_market_identical`）で補正。今回の候補は γ=1.0 なので結論に影響なし。
  (c) win Brier 0.05787 vs 現行 0.05797 → PASS
  参考: **top1 は候補・現行とも 0.331 で同一**（◎の顔ぶれは実質不変）。
  参考: CORP MCB 候補 0.00032 vs 現行 0.00045（事前コミットの副 KPI。候補の較正が
  わずかに良い＝ΔR²/Brier と同方向で、判定を変えない）。
- 機序は #703 Phase 3（最適モデル指数 ≈ 0）と一致し、ADR 0034（α∈{0.2,0.3,0.4} の
  in-sample 比較）・ADR 0045（71R）の検証条件を上回る測定で確定。

#### 却下した代替案

- **α=0 の即時採用**: クローズオッズ基準の証拠のみで本番（朝オッズ適用）の定数を変えるのは
  外挿。朝市場は緩く（「朝の +EV は直前に剥がれる」の確定知の裏面）、朝オッズ相手なら
  モデルの寄与が正でありうる。#218 で snapshots 基準の α を測ってから決める。
- **現行維持でクローズ**: 測定はゲート通過しており、live 測定（#218）を残したまま閉じると
  「α=0.2 が市場単体より有意に悪い（クローズ基準）」という確定事実が宙に浮く。保留が正。

#### 影響

- 本番挙動は不変。`mag_sweep.py` は再測定手順として残る（行数・レース集合の一致、
  忠実性アンカー、1% ガードはスクリプトが自動検査して不一致は停止する。**dump 3 本の
  SHA 統一だけは手動確認**——生成時に同一コミットでビルドしたバイナリを使うこと）。
- α=0 を将来採用する場合の同期対象（列挙）: `config.rs`
  （RECOMMENDED_MARKET_BLEND_ALPHA）・prediction/tests.rs の定数ガード・REQ-D22-001・
  CLAUDE.md「本番モデル」行・glossary `blended` 行。**D23 側への波及**: 混戦判定
  （REQ-D23-005 の band は blended 由来）・印・predict-watch 表示が市場確率ベースになる。
- eval 窓は #703 に続き本測定でも消費した（採否ゲートは fit 評価前に事前コミット済み・
  issue #284 コメント参照）。
- バックテスト履歴の記録先は betting-rule-history.md でなく本決定ログ（#652 運用・
  issue 要件からの意図的変更）。
- 関連: #284（事前コミットしたゲート定義は issue コメント参照）/ #218（live α 再校正・
  最終判断をセットで行う対）/ #703（計測器・対数プール棄却 = 機序）/ ADR 0034 /
  ADR 0045 / ADR 0052 / backtest.md 評価プロトコル / `scripts/predict-check/mag_sweep.py`
