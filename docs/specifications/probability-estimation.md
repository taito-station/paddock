...
raw_score =
    ...
    ) / Σ(present weights)           // 例: 騎手・前走あり=5.25、両方なし=4
```

> 注1: 騎手未登録馬（`jockey_surface = None`）・前走なし馬（`recent_form = None`）はその項と重みを
> **母数から除外**するため、欠落項で不当に減点されない（ADR 0007/0008）。騎手項・フォーム項は
> 「中立(0.5)からの差分」としてのみ効く。全馬が同条件のときは定数除算となり相対順位は不変。
...
- **採用 α（本番既定）= 0.3**。backtest（2026-03-01〜05-31, 144R, 市場 = `results.odds`）の α スイープ:
  ...
  > 注: このスイープは市場に**確定（クローズ）オッズ**を使うため、live 予想（事前スナップショット
  > `race_odds` を使用）より楽観的な上限値。市場オッズが無いレースは自動でモデルのみにフォールバック
  > するため、ブレンド有効化による副作用は無い。
...
- モデルは過去成績と前走フォーム（馬体重変化・前走人気乖離・前走間隔, #31）を使用。馬場状態・調教等は
  考慮しない
