...
| REQ-D22-002 | ベイズ縮約の擬似カウントは m=10（`RECOMMENDED_SHRINKAGE_M`）。backtest の既定は縮約 off なので production 再現には `--shrinkage-m 10` を明示する | `bt` から `--shrinkage-m` を外し、`--from 2026-03-28 --to 2026-05-31 --blend-alpha 0.2`（144R・ADR 0016 の窓）で m ∈ {off, 5, 10, 20, 50} を掃引。**off 比で単勝 Brier / LogLoss が改善し m=10 が最良・m=50 は過縮約で劣化**すること（0016 実測: LogLoss 0.2718 → 0.2506）。ADR 0034 の 4891R では m=5/10/20 が同値なので m の選択は決められない——そちらは「m 方向に差が出ない」ことの追認に使う | [ADR 0016](../original-docs/0016-shrinkage-and-recency.md) / [0034](../original-docs/0034-alpha-retune-recency-rejected.md) | Confirmed |
...
| REQ-D22-005 | 近走トレンドは前走のみ（`trend_n = 1`）。N=2/3 は全指標が悪化する | `bt --from 2026-03-01 --to 2026-05-31 --blend-alpha 0.2 --trend-n $n`（n ∈ {1,2,3}・893R）で N=1 を上回らないこと | [ADR 0036](../original-docs/0036-recent-form-trend-n-rejected.md) | Confirmed |
...
| REQ-D22-007 | 騎手直近フォームの重みは 0（`jockey_recent_form_weight: None`）。算出機構と `--jockey-form-weight` は再評価用に残す | `bt --from 2026-01-01 --to 2026-06-14 --blend-alpha 0.2 --jockey-form-weight $w` の掃引 | [ADR 0038](../original-docs/0038-jockey-recent-form-rejected.md) | Confirmed |
