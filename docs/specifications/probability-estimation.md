...
raw_score =
    ...
    [ + 0.25 × jockey_recent_form ]  // 騎手直近 N 走フォーム[0,1]（騎手あり×近走ありのときのみ, #221）
    ) / Σ(present weights)           // 例: 全項あり=7.75、全項なし: Σ重み=0（下記 注2）
```
...
> 重み 0.25 は暫定値（`FORM_WEIGHT` 同値）。N × weight の backtest sweep 後に ADR 0035 で確定する。
