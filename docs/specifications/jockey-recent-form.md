...
デフォルト実装は per-jockey ループ（既存 `recent_runs_batch` の既定実装パターンに準拠）。rdb-gateway のみウィンドウ関数で一括 override。
...
**実装時の注意:** `try_join!` の引数変更により `test_predict_race.rs` 等の既存テスト・mock が
コンパイルエラーになる。mock の `jockey_recent_runs_batch` 実装（空 HashMap を返す既定）を
追加すること。
...
```rust
// use-case/src/interactor/race/predict.rs および backtest.rs
// 両ファイルから参照するため use-case/src/interactor/race/mod.rs 等の共通箇所に定義する
// （既存の RECENT_RUNS_LIMIT が backtest.rs ローカル定数として重複定義されているアンチパターンの踏襲を避ける）
...
- [ ] `use-case` mock に `jockey_recent_runs_batch` の既定実装（空 HashMap を返す）を追加する
- [ ] テスト (`test_predict_race.rs` 等) が 5 タプル destructure でコンパイルが通ることを確認する
