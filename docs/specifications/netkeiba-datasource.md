...
| 単勝オッズ・人気取得 | 同ページの単勝オッズ・人気を取得し `race_odds`(単勝)へ保存 |
| race_id 構築 | 構成要素(年/場/回/日/R)からの race_id 構築を支援 |
| 文字コード | EUC-JP → UTF-8 変換を内部で吸収 |
...
| 取得項目 | 用途 | 保存先 |
|---------|------|-------|
| 枠番 | `HorseEntry.gate_num` | horse_entries |
| 馬番 | `HorseEntry.horse_num` | horse_entries |
| 馬名 | `HorseEntry.horse_name` | horse_entries |
| 騎手 | `HorseEntry.jockey` | horse_entries |
| 単勝オッズ | `race_odds`(win) | race_odds |
| 人気 | `race_odds.popularity` | race_odds |
| 芝/ダ・距離 | `RaceCard.surface` / `distance` | race_cards |
| (斤量・性齢) | — | 破棄(スコープ外) |

`RaceCard.venue` / `round` / `day` / `race_num` / `date` は race_id（12桁）から導出する。
