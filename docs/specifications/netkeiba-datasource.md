...
| 三連単 | `999999.9` | 32,973 行。issue は挙げていなかった 2 種目の番兵 |
...
| Python `scripts/predict-check/odds_guard.py` | **import 時**に読んで集合を作る | import した解析スクリプトが**起動時に**落ちる（原因を明示して停止する。空リストへのフォールバックはしない——番兵が素通りするため） |
