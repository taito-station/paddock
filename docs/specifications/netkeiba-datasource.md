...
レース前で未確定(`---.-`)の馬は除外する。netkeiba 経由(fetch-card)は `type=1`（単勝・複勝）のみ
取得する。組合せ券種（馬連・ワイド・3連複 等）は #38 で JRA odds-scraper 経由（predict の
`OddsInteractor`）で取得・永続化しており、netkeiba の `type=4` 以降は使っていない。
