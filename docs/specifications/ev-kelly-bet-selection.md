...
優先度: **馬連 > 馬単 > 三連複 > 単勝 > 複勝**（三連単は EV > trifecta_ev_threshold のみ）。  
同一馬券種内は EV 降順でソートする。
...
単一馬の `win_prob` をベースに多頭組み合わせ確率を近似する。

| 馬券種 | 確率計算式 |
|-------|----------|
| 単勝 | `win_prob[i]` |
| 複勝 | `show_prob[i]` |
| 馬連 `{a,b}` | `win[a]·win[b]·(1/(1−win[a]) + 1/(1−win[b]))` |
| 馬単 `a→b` | `win[a]·win[b]/(1−win[a])` |
| 三連複 `{a,b,c}` | 全 6 順列の三連単確率の合計 |
| 三連単 `a→b→c` | `win[a]·win[b]/(1−win[a])·win[c]/(1−win[a]−win[b])` |

Harville 公式の前提: 1 着馬が抜けた後のフィールドで各馬が独立に競う。
精度は限定的だが、EV 計算に十分な近似値を提供する。

**除算ゼロ対策**: `1 − win[i]` が極端に小さい（win_prob ≒ 1.0）場合は分母を最小値 `f64::EPSILON` でクランプする。
...
- 三連複/三連単は組み合わせ数が O(n³) となるため、出走頭数 18 頭の場合最大 816/4896 組み合わせ。パフォーマンス上の問題はないが、EV フィルタ後の推奨数が膨大にならないよう閾値設定が重要
- `kelly_cap` のデフォルト 0.25 は半 Kelly（実際の Kelly の 1/2 程度）を意図した保守的設定
