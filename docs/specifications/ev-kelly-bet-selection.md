...
| REQ-D23-002 | 相手は 3 券種とも model top5。広げない | 既定の「相手 5 頭」は ADR 0019 が置いた設計値。**広げないこと**を測ったのは 2 券種で、**3 連複**は ADR 0030 の相手幅スイープ（top5 / top7 / 全頭・混戦 90R）で回収率が下がること、**ワイド**は ADR 0065 の 262R（12 開催日）比較で top3 と top5 に有意差が無いこと。**馬連の相手幅は単独スイープ未実施**（0019 の既定のまま。測るなら `scripts/predict-check/strategy_eval.py` の相手頭数感度） | [ADR 0019](../original-docs/0019-portfolio-generator.md)（既定 5 頭）/ [0030](../original-docs/0030-konsen-trio-partner-width-rejected.md)（3 連複）/ [0065](../original-docs/0065-wide-partners-top5-alignment.md)（ワイド） | Confirmed |
