...
⚠ このレースは発走済みです（発走 10:40 / 判定時刻 08-16 14:22）。
...
- **プロンプトは発走時刻と判定時刻を併記する**。上記のとおり見出しと食い違いうるうえ、
  ...
  判定される**。「なぜ聞かれたか」の手掛かりは文面しか無いので両方出す。日付を含めるのは、
...
- **判定の second source は作らない**。post_time の引き当てと `is_started_at` の呼び出しは
  ...
  （`race_heading_for_day`）と確認（`confirm_record_if_started`）の両方がそこを通る。
  一致は unit テストが張る（一致だけでなく**各ケースの期待値そのもの**も張る——一致だけを見ると
  判定関数を壊しても見出しが道連れで壊れてテストが素通りする）。
- **`e` で入力した金額は「記録しない」を選ぶと破棄される**。次にそのレースを扱うときはレース先頭
  （馬場入力・オッズ表示）からやり直しになる。安全側に倒した結果で、意図的な挙動。
- **過去日の遡り入力では全レースで確認が出る**（開催日が過ぎていれば時刻を見ずに発走済み）。
  記録するレースの数だけ打鍵が乗るが、`s` で流すレースには乗らない。
