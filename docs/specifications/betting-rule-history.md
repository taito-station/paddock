...
- 結果: **ROI≥100% は損益分岐そのもの**で、下げる＝−EV を承知で買う＝長期で負け。平均 ROI 73.1% は JRA 控除率（約 20〜27.5%）由来のフェア ROI ≈ 73〜80% とほぼ一致し、モデルは平均で市場と等価＝較正済み。+EV は控除率を食い破る稀な歪みだけで構造的に稀＝買い目が来ないのは仕様どおり。閾値直下に「買うべきレース」が溜まる兆候は無し（結果のある 71R の実現 ROI も baseline 83.9〜84.0%＝ADR 0028 で 100% 未満）。
- **結論**: ROI≥100% を維持。閾値引き下げは不採用。唯一意味のある検証＝推定 ROI の較正（予測 vs 実現）は、ワイド+結果が同居するレースが DB に1本も無く現状実行不能 → #248 のデータ蓄積後に #249 で実施（保留）。
