...
### ⑧ 馬単（exacta）特化・順序プレミアム（要再検討 / 2026-06-27 / ADR 0043）
...
- **結論**: 順序プレミアムは #246 較正後の 71R では model EV シグナル経由で取り出せない。馬連→馬単の置換は無効化すべき（→ #271 で revert 提案）。馬単オッズ取得・診断表示の下地は将来の再検証用に温存。
- 詳細: [docs/adr/0043-exacta-in-portfolio-adopted.md](../adr/0043-exacta-in-portfolio-adopted.md)「71R 検証（#262）」（関連 Issue: #262, #252, #246, #271）
