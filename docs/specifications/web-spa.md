...
[Issue #34](https://github.com/taito-station/paddock/issues/34) / 依存: [#33 REST API](https://github.com/taito-station/paddock/issues/33)
...
- 日付を選び、その開催日のレース一覧を表示（venue / レース番号 / 距離 / 芝ダ）。
...
> 認証本体（JWT/argon2）はマルチユーザー化の専用 Issue で `~/.claude/rules/rust/architecture.md` の認証パターンに従って実装する。本フェーズでは作らない。

---

## API エンドポイント（#33 で実装、本 SPA が消費）

read 系（#33 スコープ）に加え、セッション操作の write 系が必要。**#33 の現スコープは read 系のみ**のため、セッション write 系は本仕様を根拠に #33 へ追記するか別 Issue で切り出す。
...
| GET | `/analyze/{horse\|course\|jockey}` | 分析統計 | #33, #50 |
| GET | `/sessions/{date}` | セッション収支 + 明細（summary） | #33 |
| POST | `/sessions/{date}` | セッション新規作成（budget 指定） | 新規 |
| POST | `/sessions/{date}/races/{race_id}/outcome` | 賭け金・払戻を記録（残高更新, 1tx） | 新規 |
...
- #33 REST API（前提・read 系 + セッション write 系の追記要否を判断）
- #34 本 SPA
