...
2. **買い目推奨**: 各買い目を券種・組合せ・EV・推奨額で表示。本番配分は `build_portfolio`（ワイド・馬連・三連複の◎軸ながし、確率重み＋最大剰余法・100 円単位。[ADR 0019](../adr/0019-portfolio-generator.md)）。Kelly 配分は [ADR 0054](../adr/0054-kelly-staking-rejected.md) で棄却済みで、`select_bets`/Kelly は backtest 評価専用。
