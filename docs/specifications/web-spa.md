...
2. **買い目推奨**: 各買い目を券種・組合せ・EV・推奨額で表示。本番配分は `build_portfolio`（ワイド・馬連・三連複の◎軸ながし、券種予算を 100 円単位で均等配分。[ADR 0019](../adr/0019-portfolio-generator.md)。券種内の均等配分は [ADR 0046](../adr/0046-allocation-prob-weight-no-floor-rejected.md) で確率重み化を棄却し維持）。Kelly 配分は [ADR 0054](../adr/0054-kelly-staking-rejected.md) で棄却済みで、`select_bets`/Kelly は backtest 評価専用。
   ...
   **朝↔現比較（#448 / ADR 0055/0060 の可視化）**: `/board` レスポンスの `morning_at` が非 `null` のとき、盤に「朝比較」列と ROI サマリを表示する。
   - 各馬の列に **`morning_win_odds`**（朝単勝）と `win_odds`（現単勝）の差分を「▲人気化（現 < 朝）／△妙味（現 > 朝）」として表示。
   - 盤サマリに **`morning_roi → roi`**（朝 ROI → 現 ROI）を並列表示（「朝 +EV が発走直前に剥がれる」を数値確認できる）。
   - **トグル（既定 ON）** で非表示可。非開催・単複のみ履歴（`morning_at=null`）は自動縮退（比較列を出さない）。
   - 軸（◎）は朝比較によって変更しない（軸ロック思想の体現。単なる表示機能であって意思決定の変更を促さない）。
...
| GET | `/races/{race_id}/board` | ライブ盤（全頭＋買い目＋朝↔現比較） | #33, #51, #448 |
