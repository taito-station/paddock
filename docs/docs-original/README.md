...
> 例外は 2 つだけ。(1) **GitHub Issue 本文の転記章の削除**（ADR 0074。実績は `382`/`384`/`389`/`401` の 4 本）。
...
issue 本文や外部資料を**逐語転記**する層なので、ADR 草案を含む issue をそのまま置くと
本文構造（コードフェンスの外の行頭に `## ステータス` と `## 決定` が同居）で判定に
引っかかり、CI と pre-push が落ちる。転記内容は書き換えない（RO）ので、その場合は
...
これは本文ではなく判定用のメタデータなので、RO 原則の例外として認める。
