...
一次資料: [docs/original-docs/0082-sources-coverage-checks.md](../original-docs/0082-sources-coverage-checks.md)
...
- 反映先: ADR 0082 決定 3 / `docs/knowledge/doc-classes.md`
...
- 反映先: ADR 0082 理由 / `docs/knowledge/doc-classes.md`

## Q3: error か warning か

- 観測/根拠: 既存 10 項目のうち warning は「充足ギャップ」「サブディレクトリの .md」「履歴を辿れず
  判定できなかった」の 3 つだけで、いずれも**不変条件そのものではない**。#580 は stale を
...
- 回答: **確定。両方 error。** 現状の違反が 0〜1 件（しかもその 1 件は既知の例外）なので、
  ...
  数えない（CI では使わない）。
- 反映先: ADR 0082 決定 1・2 / `scripts/check-doc-classes.py`
...
- 反映先: ADR 0082 決定 2・却下案 / `check_req_blocks()`
...
- 反映先: ADR 0082 決定 4 / `scripts/check-doc-classes.py`
...
- 回答: **確定。1 PR・2 コミット（`Closes #596, #597`）。** トピックは
  ...
  bisect 可能性は保つ。ADR も 1 本（0082）にまとめる——2 つの検査は同じ穴の両側なので、
  決定としては 1 つ。
- 反映先: 運用のみ（文書に残す確定知ではない）

## 補足: ADR 番号の採番衝突（実地の記録）

本作業中、**別セッションが同時刻に #606 用の ADR 0081 をコミットした**ため、当初 0081 を
充てていた本 ADR を **0082** へ採番し直した。`scripts/check-adr-numbers.sh next` は
実行時点のスナップショットしか返さないので、**並走セッションがあるときは commit 直前に
再確認する**（pre-push の `check-adr-numbers.sh` が重複を検出するので取り違えは残らない）。
