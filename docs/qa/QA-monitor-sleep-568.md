...
- 回答: **確定。5 秒**（当初 30 秒としたが棄却した）。`tokio::time::sleep` は単調時計基準で、
  ホストのスリープ中は進まない＝ティックは「起きている時間」でしか消化されない。一次資料の
  2026-08-01 の Standby は **7 秒 DarkWake が 4 回＝累計 28 秒**で、30 秒刻みでは 1 ティックも満了せず
  再スイープに到達しない（＝修正しても実事象を救えない）。5 秒なら DarkWake 1 回ごとに満了する。
  空回りは既定間隔で 60〜180 回/スイープ、1 回あたり時刻比較のみなので無視できる。
  ただし「DarkWake 1 回で満了する」は実機の Standby では未検証（実機検証は `pmset sleepnow` からの
  本格復帰）。次の開催日の実運用ログで確認する。
- 反映先: docs/knowledge/monitor-loop-sleep-resilience.md
...
- 観測/根拠: レビュー 5 巡を経て、指摘の多くが「監視プロセス自身の `caffeinate` 確保」と
  「日付判定の JST 化」に起因していた。前者は keep_awake モジュール（78 行）＋関数注入 seam ＋
  遅延確保 ＋ `try_wait` 生存監視 ＋ macOS 専用 CI ジョブ ＋ 専用テストを芋づるで要求し、後者は
  コードとテストの時刻基準を割って CI を毎日 9 時間 red にした。
- 回答: **確定。どちらも持たない**。
  - **抑止**: 8/1 の起点は蓋閉じ（clamshell sleep）で `caffeinate -i` の守備範囲外＝**自前で持っても
    実事象は防げない**。#264 が既に同じ役割を担っており、二重実装は ADR 0064 が戒める second source。
    issue #568 の要件文も「内部で確保する、**あるいは起動手順に組み込む**」と後者を許容している。
  - **日付**: 運用ホストは JST 固定で非 JST ホストは存在しない。前提の点検は既存の
    `warn_if_not_today_jst` が起動時に行う。判定はローカル日付のままにする。
  骨格の責務は「wall-clock 基準で寝ても壊れない」ことだけに絞る。
- 反映先: docs/knowledge/monitor-loop-sleep-resilience.md / ADR 0072（却下した代替案）
