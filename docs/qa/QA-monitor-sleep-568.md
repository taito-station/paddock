...
- 回答: **確定**。15:49 就寝 → 16:20 復帰で
  `⚠ スイープが 30 分途切れました…` の警告が出て、**その直後にスイープ #3 が走った**（即時 catch-up）。
...
- 回答: **確定。30 秒**。最短間隔 1 分に対して十分細かく、通常運用での空回りは 1 スイープあたり
  高々 2 回。復帰検知の遅れも最大 30 秒で、発走直前 EV の判断粒度（分オーダー）に影響しない。
- 反映先: docs/knowledge/monitor-loop-sleep-resilience.md
