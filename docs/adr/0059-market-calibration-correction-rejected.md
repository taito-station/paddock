# 0059. 市場は効率的（人気-穴バイアス不在）＝市場較正補正による EV 取得を棄却
...
全体 n=61,821・単勝 blind ROI=0.736。

## 決定

市場較正補正による EV 取得を**採用しない**。JRA/netkeiba の単勝市場は takeout 後で効率的で、較正補正で取れる系統的 edge が無い。

## 理由

- **人気-穴バイアスは実質不在**。古典的パターン（本命を過小評価＋大穴を買われすぎ＝オッズが上がるほど実勝率が implied を単調に下回る）が出ていない。特に大穴帯 50+（n=21,977）の差が **−0.001＝較正済み**（バイアスがあればここが大きくマイナスになるはず）。差は全帯 ±0.006 のノイズ内。唯一の外れは超本命 1-1.5 帯の +0.039 だが n=221 と薄く、方向も「本命がやや過小評価」で古典バイアスと逆・単調でない。
- **単勝 ROI は全帯 < 1.0**（0.62-0.90）。**張って +になる帯は無い**＝較正補正で抜ける free EV は無い。best は 20-30 帯の 0.902 だが依然マイナスでノイズ。
- ADR 0027（市場＝公開データの最良推定）を、純モデル resolution に続き**市場自体の較正**でも再確認。
