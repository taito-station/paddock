...
- read 系の use-case / Repository は**既存実装を再利用**し、新規追加は interface / apps 層に閉じる。
...
- **既存ロジック再利用**: 確率推定・分析は use-case に集約済みで、interface/apps を足すだけで API 化できる。レイヤー責務を崩さない。
