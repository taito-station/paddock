...
- **新規**: Postgres テーブル `live_ev_snapshots`（マイグレーション）／`live_ev.py --emit-json`（＋テスト）／`refresh_ev.sh` に永続化ステップ／read API `GET /api/live/{date}`（rest-controller・use-case・rdb-gateway・api-server の 4 層）＋ **OpenAPI を一級成果物とする**（utoipa コードファースト＋`openapi.json` スナップショット更新・検証を DoD 化）／SPA `LiveBets` ビュー 1 画面。
