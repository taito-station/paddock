...
- 対処: 過去レースなので着順は netkeiba 結果ページ（UTF-8）から直接取得し、DB の snapshot オッズと突合。**94R 全て結果取得成功・欠落 0**。

## 検証方法

`scripts/predict-check/late_money_probe.py`（新規・standalone・標準ライブラリのみ）:
...
| （参考）Spearman(終値人気, 着順) | −0.548 | だが①は交絡: 締まる馬＝人気馬（drift 五分位で終値 implied が 0.016→0.164 と単調上昇） |
...
- ④ が決定的: `logit(p_last) + b·drift` を b について in-sample グリッド探索（[−3,+3]）しても、logloss を下げる b は
  **厳密に 0**。過学習の自由度を与えてすら drift は終値を改善できない。
...
drift に終値を超える増分予測力は無い**（ロジスティックで drift の最適重み＝0）。市場効率の教科書的帰結であり、
...
- `scripts/predict-check/late_money_probe.py`（新規）: DB snapshot→first/last drift 算出→netkeiba 結果 scrape（ローカル
  キャッシュ）→level 制御下の相関・五分位残差・1 変数ロジ logloss。再実行可能・依存は標準ライブラリのみ。
