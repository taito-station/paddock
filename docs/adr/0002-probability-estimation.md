...
2. **`GroupStat` に `shows: u32`（複勝カウント）を追加する。**  
   既存の `places`（連対、top-2）に加え `shows`（複勝、top-3）を追加する。
   スキーマ変更ではなく既存クエリへの集計カラム追加で対応する。

3. **スコアリング重みは固定値とする。**  
   `course × gate(×2) + horse_surface(×1) + horse_distance(×1) + jockey_surface(×1)`  
...
- `GroupStat` の全コンストラクタと SQL クエリを更新する必要がある
  （horse_stats × 5 パターン、course_stats × 1 パターン、jockey_stats × 3 パターン）
