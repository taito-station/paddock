...
Issue #12 で Domain 層の EV 計算・Kelly 配分ロジックが完成した。
...
- セッション状態（残高・累計）は App 層のみで管理し、Domain/Use-Case 層には持ち込まない
...
- `find_races_by_date` は `races` テーブルの日付インデックスを使った単純なクエリで追加コストが小さい
- `find_race_odds` は既存の odds-scraper が書き込む `race_odds` テーブルから読むのみ（Schema 変更なし）
