...
α=0.3 が校正最良（LogLoss 最小）、α=0.5 は的中率↑だが校正↓。本番 α=0.3+m=10 が実用上ベスト。
...
- 本番予想が実運用で機能している（live 単勝的中率の実績）のは、この市場ブレンドを既に使っているため。
...
- メモリ: `project-predict-check-workflow`（本番 α=0.3 / m=10、live 実測精度）
