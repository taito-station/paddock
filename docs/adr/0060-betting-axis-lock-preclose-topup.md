...
純モデルの resolution は現行＋取得可能データで天井（ADR 0058：重み/within-race/recency/クラス/血統/coverage すべて棄却＝factor 冗長性、ADR 0059：市場の人気-穴バイアスも sub-takeout で exploitable でない）。「**市場より上手く勝者を当てる**」路線は closed（ADR 0027 と整合）。
...
2. **直前オッズはズレ増額のみ**: 発走直前オッズの用途は、軸の馬が自モデル確率より美味しくズレた（過小人気）ときの **増額 / 点数追加** に限る。不利側にズレても軸は動かさず、レース全体の見送り（そのレースを張らない）判断だけを許す。
3. **軸フリップ禁止**: "妙味" や直前ブレンドで高確信の軸を別馬へ乗り換えない。◎ の見直しは、事前根拠を崩す**新情報**（発表された取消・馬場激変・重大な馬体/パドック異常等）が出た時のみ、理由を明示して行う。
4. **ライブ監視規律を decision-support に整合**: predict-watch は自動 go/no-go でなく判断材料の提示（ADR 0055）。参考 ROI は増額候補の検出に使い、張る/見送りと増額は人間が決める。監視中の 1 行明示は「軸（不変）＋増額候補/据え置き/レース見送り」を述べ、**軸そのものはここで動かさない**（旧「◎ が変わったらフリップ警告」＝軸が動く前提の規律を廃し、軸不変を前提にした規律へ差し替え）。
...
- 予算・配分（¥5,000・3券種・混戦判定＝ADR 0046 等）・相手の広さ（top5）は不変（バックテスト済み・本 arc の対象外）。
...
- 関連: 0055（EV 層分離・本 ADR はその運用面 follow-up）/0044（model-EV ゲート逆予測）/0058・0059（resolution 天井・市場較正棄却）/0027（精度の主レバー＝市場ブレンド）/0046（配分）。
