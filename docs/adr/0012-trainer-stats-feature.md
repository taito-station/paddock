...
### #87 での再検証
JRA から結果データを再取得して測定 DB に **trainer 母数を充足**（476 レース / trainer 充足 99%）し、
ADR と同じ window で `TRAINER_WEIGHT` をスイープ再検証した（**#81 後の None 母数除外ロジック**上、
2026-03-28〜05-31 / 144 レース）:

| TRAINER_WEIGHT | 単勝 | 連対 | 複勝 | 回収率 | Brier(単) | LogLoss(単) | Brier(複) |
|---|---|---|---|---|---|---|---|
| 0.0 | 10.4% | 16.7% | 28.5% | 36.1% | 0.0657 | 0.6006 | 0.1683 |
| 0.5 | 11.1% | 17.4% | 28.5% | 39.5% | 0.0638 | 0.4013 | 0.1604 |
| **1.0** | 9.7% | 15.3% | 27.8% | 38.9% | 0.0635 | **0.3998** | **0.1595** |
| 2.0 | 10.4% | 16.7% | 31.2% | 43.3% | 0.0634 | 0.4004 | 0.1598 |

### 結論
- trainer 項は **0.0（無効）比で校正が明確に改善**（LogLoss 単勝 0.60→0.40、Brier も低下）＝調教師
  シグナルは有効。母数充足により before=after が解消し、項が実際に発火することを確認した。
- 0.5 / 1.0 / 2.0 は校正がほぼ拮抗。**1.0 が LogLoss 単勝・Brier 複勝で最良**（Brier 連対も同率最良）。
  的中率・回収率は 144 レースのノイズが大きく重み選定の根拠にしない。
