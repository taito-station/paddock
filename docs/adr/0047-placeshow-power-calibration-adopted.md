...
- **回収率リスクはゼロ**（win 不変由来。show/place は表示のみで EV 非関与）。show は magnitude 校正で順位を概ね保つが、place/show を別々に正規化したのち各馬内で max を取り単調化するため、接近した馬の前後は入れ替わりうる（厳密なランク不変ではない）。過学習リスクは単一パラメータ冪なので小。
...
- 校正窓は純モデル（α=0, `gen_pure_preds --blend-alpha 0`）の place/show。production は α=0.2 ブレンド後に同じ γ を適用するため、表示値は校正窓と僅かにズレうる（win ブレンドで place/show が単調再是正で押し上がる分）。表示専用・影響軽微。
