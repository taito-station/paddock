...
- **win は byte 完全一致**（`pure_preds.tsv` vs 再生成 `pure_after.tsv` の win 列 diff なし）→ top-1 31%・win ECE 1.39% 不変。**EV/買い目/回収率は win 由来のため不変が証明される**（umaren ROI を別途回す必要なし）。
- **show ECE 9.34%→7.04%**（掃引予測 7.08% と整合）、place ECE 6.6%→3.8%。show の圧縮が是正され強い複勝馬に高い show が付く（60%+ 帯が populated）。
- `apply_placeshow_power` の単調性保持・ランク保存・シャープ化方向・no-op をユニットテストで担保。

## 理由

- **単調変換なのでランクは不変**（どの馬が来やすいかの順序は変えず、magnitude だけ校正）＝回収率リスクゼロ・過学習リスク小。
