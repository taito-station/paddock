...
4. **監視プロセス自身がアイドルスリープを抑止する**。**最初の日付判定を通過した後**に
   `caffeinate -i -w <自分の pid>` を spawn する（macOS のみ・`--once` では確保しない。
   過去日指定で「確保しました」→即「対象日を過ぎました」という無意味なログ対を作らないため）。
...
- **トレードオフ**: 時間軸を wall-clock に移したことで、NTP のステップ補正や手動の時刻/TZ 変更に
  ...
  されない**（この方向では「必ず警告する」が成立しない）。単調タイマーにはこの弱点が無かったが、
...
- **`classify` に渡す時刻の取得点が変わった**。従来は `load_slots` 完了後、現在はサイクル開始時
  ...
  （ms オーダー）ぶん。窓 40 分・間隔 5 分のスケールでは実害はないが、`Sweeper` シグネチャ不変を
  謳う本 PR で唯一の観測可能な意味変化なので記録する。
- **macOS 分岐は専用の CI ジョブで検証する**。他ジョブは全て ubuntu で、`caffeinate` を spawn する
  `#[cfg(target_os = "macos")]` 側は Linux では no-op 分岐しか通らない。実際に動く唯一の
  プラットフォームで腐らせないため、`macos-latest` で monitor-loop だけ clippy / test する軽量ジョブを足した。
...
- **復帰検知の粒度は「ホストが起きている時間」で数える**。`tokio::time::sleep` は単調時計基準で、
  macOS ではスリープ中に進まない。したがって「期限超過なら最大 30 秒で再開」が成り立つのは
  **本格復帰（蓋を開ける等）してから**であり、7 秒 DarkWake を繰り返すだけの Standby では
  30 秒ぶんの awake 時間が貯まるまで再開しない（8/1 の一次資料がまさにこの状態）。
