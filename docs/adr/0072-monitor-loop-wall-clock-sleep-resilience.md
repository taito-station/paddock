...
1. **待機を wall-clock 期限に対する刻み待ちにする**。次スイープの期限（`Local::now() + interval`）を
   ...
   ＝ **即座に次スイープが走る**（自動再開）。刻み幅 30 秒は最短スイープ間隔 1 分に対して十分細かく、
   通常運用の空回りは 1 スイープあたり高々 2 回。
2. **途切れを検知して必ず警告する**。待機の実経過を測り、想定間隔の 2 倍を超えていたら
   `detect_sweep_gap` が検知して `⚠ スイープが N 分途切れました…` を出す。沈黙のまま監視が
   止まっていた事実を必ずログに残す。
3. **wall-clock の日付で終了する**。`should_stop_by_date(対象日, 現在日)` を継続判定に加え、
   対象日を過ぎたら時刻軸の判定に関わらず終了する。判定は**スイープ後**に置き、
   「1 スイープも走らずに終わる」経路を作らない。
...
- **追加**: `monitor_loop::detect_sweep_gap`（途切れ判定・純関数）/ `monitor_loop::should_stop_by_date`
  （日付終了判定・純関数）/ `monitor_loop::keep_awake`（macOS の caffeinate 確保）。
  `driver` 内に `sleep_until_with_tick` / `wait_next_sweep`。
...
- **新しい終了経路**: 対象日を過ぎると
  ...
  過去日を `--date` に渡す検証用途では **1 スイープだけ走って終了する**（従来は無限ループしていた）。
