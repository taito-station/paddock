...
※ 「なし」=従来 Harville（市場フィルタ無し）の素の点数。点数は exotic_alpha に対し単調でない（α=0.5 が最少）——市場重みが増えるほど long-shot 組が EV 閾値（ev>1.0）を割って落ちるが、curation（券種別上限・min_kelly）との相互作用で閾値割れの量は α に厳密単調でないため。いずれにせよ全 α で的中 0%。
...
- 不変の本番経路: `build_portfolio`（`src/domain/src/portfolio/mod.rs`）/ `simulate`（`src/domain/src/simulation/mod.rs`）。本番 predict（`apps/predict`）・`recommend_bets`（`src/use-case/src/interactor/race/recommend.rs`）の双方が `build_portfolio` 経由
