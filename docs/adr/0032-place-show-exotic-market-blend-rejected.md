...
#196（backtest 高速化, PR #203）で 450R が約46分→4分になりスイープが現実的になったため、以下を**実装して
計測した**（measurement ordering 準拠: 挙動変更 → before/after 計測）。
...
| 複勝（place）買い目 回収率 | 79.9%（236点・的中10.2%） | **49.6%**（174点・的中8.0%） | **悪化** |
...
- **Phase 1**: #194 の主目的（複勝系の校正）は達成できるが、買い目予算が乗る複勝の回収率が悪化したため
  ...
  楽観アーティファクト（小頭数（7頭以下）の複勝は採用確率＝show（3着以内）と的中定義（2着以内）の
  非対称で平均予測が実的中率を上回りやすい。`paddock-analyze backtest` の by_exotic 出力末尾に脚注として
  既出）を割り引くと本番化を正当化できない。
...
- 不変の本番経路: `build_portfolio`（`src/domain/src/portfolio/mod.rs`）/ `simulate`
