...
- **障害レースは取り込み対象外**。`exit 0` で終わり stdout に `スキップ: 障害レースは取り込み対象外です` を出す（取り込み失敗ではない・ADR 0075）。**開催日の取得件数が netkeiba 一覧より 1〜2 件少ないのはこれが理由**——`/api/races` の件数と突き合わせて慌てない。`exit≠0` は本物の失敗だけ
