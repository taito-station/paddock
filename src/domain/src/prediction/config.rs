...
/// 本命ギャップは +22.9pt とまだ大きく過補正の手前で、安全側。γ≥2.5 は当初 #286（win 側
/// m×recency×form の joint retune）が place/show 素スコアを作り直す前提で保留していたが、その
/// retune は 4891R 検証で素スコアを脱圧縮できず棄却された（ADR 0049）。よって γ≥2.5 は #286 に
/// ブロックされない独立の最適化課題（過補正リスクの確認が要るため未掃引）。詳細は ADR 0047 / 0049。
