...
/// - それ未満は「当日の ROI<100 のレース群」に対する分位で分ける。上位帯（p66 以上）を `Close`、
///   中位帯（p33 以上）を `Watch`、下位帯（p33 未満）を `Hidden`。分位は nearest-rank のため各帯の
///   厳密な件数比は母集団で前後するが、load-bearing な「下位 1/3 だけ Hidden＝常に上位 2/3 が在庫」は
///   満たす。これで固定 90 floor による「毎日空」を避ける（ADR0040 の ROI 分布が 75–85 に偏る前提への対処）。
