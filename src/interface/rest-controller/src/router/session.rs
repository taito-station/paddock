    ...
    S: ResultPageFetcher + Send + Sync + 'static,
{
    cfg.service(
        web::scope("/sessions")
            .route(
                "/{date}",
                web::post().to(handler::session::create_session::<R>),
            )
            .route(
                "/{date}",
                web::get().to(handler::session::get_session_summary::<R>),
            )
            .route(
                "/{date}/races/{race_id}/outcome",
                web::post().to(handler::session::record_outcome::<R>),
            )
            .route(
                "/{date}/races/{race_id}/odds:refresh",
                web::post().to(handler::session::odds_refresh::<R, O>),
            )
            .route(
