...
fn parses_quinella_normalising_order() {
    ...
    assert_eq!(map.len(), 3);
    let key = Pair::new(hn(2), hn(1)).unwrap(); // unordered: same as 1-2
...
fn parses_exacta_preserving_order() {
    ...
    assert_eq!(map.len(), 3);
    let one_two = OrderedPair::new(hn(1), hn(2)).unwrap();
    let two_one = OrderedPair::new(hn(2), hn(1)).unwrap();
...
fn parses_trio_unordered() {
    ...
    assert_eq!(map.len(), 3);
    let key = Triple::new(hn(3), hn(1), hn(2)).unwrap(); // unordered: same as 1-2-3
...
fn parses_trifecta_with_thousands_separator() {
    ...
    assert_eq!(map.len(), 3);
    let key = OrderedTriple::new(hn(3), hn(2), hn(1)).unwrap();
